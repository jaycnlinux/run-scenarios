...
# 脚本存在问题：
# 1.同时加压和串行加压，在main函数里需要优化
# 2.为啥先sar，后netperf就出问题，netperf打了双倍的流，需要分析
# 3.切换成socket模式，避免scp方式数据拷贝
...
# 全局变量 源IP、目标IP列表，流数
# 注意，传入的可能是EIP
SRC_HOST = '192.168.80.11'
DST_HOST = '192.168.80.12'
PASSWORD_HASH = '123'
...
class TestParam(object):
    ...
    def __init__(self, src_host=None, dst_host=None, test_type='udp_stream', base_port=7001,
                 flows=16, set_mb=-1, set_kpps=-1, test_time=3600, pkt_len=64):
        """
        # 测试参数
            ...
            for i in range(0, len(src_host)):
                ...
                for j in range(0, self.param.flows):
                    ...
                    c_cmd = 'netperf -H %s -p %d -t %s -l %d -- -m %d 2>/dev/null 1>/dev/null &' \
                            % (host_vpc, port_index, test_type, self.param.test_time, self.param.pkt_len)
...
def main():
    ...
    global DEBUG_MODE
    global logger
    task_app = ['iperf3', 'ping', 'qperf', 'memcached', 'netperf']
    ...
    if 'netperf' in task_app:
        ...
        netperf_param1 = TestParam(None, None, 'TCP_STREAM', 7001, 0, -1, 0, 7200, 1440)
    ...
    if 'iperf3' in task_app:
        ...
        for pps in range(0, 10, 4):
            iperf3_param1 = TestParam(None, None, 'udp', 8001, 0, -1, set_kpps=pps, test_time=3600, pkt_len=64)
        ...
        for bw in range(0, 500, 100):
            iperf3_param2 = TestParam(None, None, 'tcp', 8001, 0, bw, set_kpps=-1, test_time=3600, pkt_len=0)
