...
# version: v0.8
# 修改内容：
# 20190414 增加EIP自动识别为VPC_IP
# 脚本存在问题：
# send_sum计算平均去了，要改成求和的,如果send_sum或recv_sum为空，则打印None，多次repeat之间的send_sum不必打印
#
# 同时加压和串行加压，在main函数里需要优化
# TCP_CRR、TCP_RR等依赖原始输出的测试没法解决
...
# 全局变量 源IP、目标IP列表，流数
# 注意，传入的可能是EIP
SRC_HOST = '192.254.4.223 192.254.0.255'
DST_HOST = '192.254.14.18 192.254.9.18'
DEBUG_MODE = False
FLAVOR_TYPE = 'C3NE'
PASSWORD_HASH = '123'
...
class TestParam(object):
    """    测试参数类    """
    def __init__(self, src_host=None, dst_host=None, test_type='udp_stream', base_port=7001, flows=16, flow_cpu_ratio=0,
...
class NetperfTester(object):
    ...
    def make_netperf_cmd(self, src_host, dst_host):
        ...
        if len(src_host) == len(dst_host):
            for i in range(0, len(src_host)):
                ...
                if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                    ...
                    self.param.flows = Public.select_flow_from_host(src_host[i], vcpu_ratio=ratio)
                for j in range(0, self.param.flows):
                    # 转换VPC_IP
                    host_eip = dst_host[i]
                    host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
                    netperf_log = self.log_format % (host_eip, port_index)
                    if host_eip not in self.log_list.keys(): self.log_list[host_eip] = []
                    self.log_list[host_eip].append(netperf_log)
                    c_cmd = 'netperf -H %s -p %d -t %s -l %d -- %s %d > %s &' \
                            % (host_vpc, port_index, test_type, self.param.test_time, netperf_method, self.param.pkt_len, netperf_log)
        ...
        # 1:N
            ...
            if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
                self.param.flows = Public.select_flow_from_host(src_host[0], vcpu_ratio=ratio)
            ...
            for i in range(0, len(dst_host)):
                ...
                netperf_log = self.log_format % (host_eip, port_index)
                if host_eip not in self.log_list.keys(): self.log_list[host_eip] = []
                self.log_list[host_vpc].append(netperf_log)
            ...
            for j in range(0, left_flow):
                ...
                host_eip = dst_host[-1]
                host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
                netperf_log = self.log_format % (host_vpc, port_index)
                if host_eip not in self.log_list.keys(): self.log_list[host_eip] = []
                self.log_list[host_eip].append(netperf_log)
                c_cmd = 'netperf -H %s -p %d -t %s -l %d -- %s %d > %s &' \
                        % (host_vpc, port_index, test_type, self.param.test_time, netperf_method, self.param.pkt_len, netperf_log)
        ...
        # N:1
            ...
            if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
                self.param.flows = Public.select_flow_from_host(dst_host[0], vcpu_ratio=ratio)
...
class QperfTester(object):
    ...
    def make_qperf_cmd(self, src_host, dst_host):
        ...
        for i in range(0, len(src_host)):
            ...
            host_eip = dst_host[i]
            host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
            c_cmd = 'qperf %s -t %d -m %d -vu %s > /root/%s_qperf.log &' \
                    % (host_vpc, self.param.test_time, self.param.pkt_len,
    ...
    def fetch_data(self):
        ...
        dst_dir = '/tmp/'
        dst_file = '%s%s_qperf.log' % (dst_dir, self.param.src_host[0])
        if DEBUG_MODE:
            logger.log('call_qperf: begin collect data')
            logger.log('%s%s_qperf.log' % (dst_dir, self.param.src_host[0]))
        os.system("script -q -c 'scp %s:/root/%s_qperf.log %s' >/dev/null" %
...
class PingTester(object):
    ...
    def _make_cmd(self):
        ...
        host_eip = self.dst
        host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
        c_cmd = ['ping %s -c %d -i %i > /root/%s_ping.log &' % (host_vpc, self.count, self.interval, self.src)]
...
def main():
    ...
    global DEBUG_MODE
    global logger
    #task_app = ['netperf', 'qperf', 'ping', 'memcached', 'iperf3_bg']
    task_app = ['iperf3_bg', 'netperf']
    ...
    if 'netperf' in task_app:
        logger.log('task_app: netperf')
        # 先来一波TCP_STREAM带宽测试
        netperf_param1 = TestParam(None, None, 'TCP_STREAM', 7001, 0, 0, -1, 0, 10, 1440)
        netperf_param1.set_param(src_host=src_host_list, dst_host=dst_host_list)
        netperf_tester1 = NetperfTester(netperf_param1)
        netperf_tester1.sar.set_param(src_host=src_host_list, dst_host=None, c_time=10, details=True, eth=NIC_NAME)
        run_netperf_iperf3_task(netperf_tester1, repeat)
        # 再来一波TCP_CRR
        # netperf_param2 = TestParam(None, None, 'TCP_CRR', 7001, 0, -1, 0, 60, 64)
        # netperf_param2.set_param(src_host=src_host_list, dst_host=dst_host_list)
        # netperf_tester2 = NetperfTester(netperf_param1)
    if 'iperf3' in task_app:
        logger.log('task_app: iperf3')
        # 先测一波梯度pps，单位K
        ...
        for bw in range(0, 500, 100):
            ...
            iperf3_test2.sar.set_param(src_host=src_host_list, c_time=10, details=True, eth=NIC_NAME)
            run_netperf_iperf3_task(iperf3_test2, repeat)
    if 'iperf3_bg' in task_app:
        # 背景加压
        logger.log('task_app: iperf3_bg')
        ratio = 0.1
    ...
    if 'qperf' in task_app:
        ...
        qperf_test2.param.set_param([src_host_list[0]], [dst_host_list[0]], 'tcp_lat', test_time=10, pkt_len=64)
        run_qperf_task(qperf_test2, repeat)
    if 'ssh' in task_app:
        logger.log('task_app: ssh')
        run_ssh_login_and_get_time([OB_HOST], src_host_list, test_time=60, sleep_gap=2, test_round=2)
    if 'ping' in task_app:
        logger.log('task_app: ping')
        ping_tester1 = PingTester(src=src_host_list[0], dst=dst_host_list[0], count=60, interval=1.0)
        run_ping_task(ping_tester1, repeat)
    if 'memcached' in task_app:
        logger.log('task_app: memcached')
