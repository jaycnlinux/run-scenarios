...
# date: 20190411
# 修改内容：
# 20190411 修复0pps,exec_shell_command的时候，判断是否为本机，如果是本机，无需ssh
# 脚本存在问题：
# 1.-b=0是不加压
# 2.同时加压和串行加压，在main函数里需要优化
# 3.为啥先sar，后netperf就出问题，netperf打了双倍的流，需要分析
# 4.切换成socket模式，避免scp方式数据拷贝
...
SRC_HOST = '192.168.1.108'
DST_HOST = '192.168.1.54'
...
class Iperf3Tester(object):
        ...
        if len(self.param.src_host) == len(self.param.dst_host):
            ...
            for i in range(0, len(self.param.src_host)):
                if self.param.flows == 0:
                    ...
                    logger.log('select flows = %d, kpps=%d' % (self.param.flows, self.param.set_kpps))
                # 根据PPS选择带宽
                if self.param.set_kpps > 0:
                    self.param.set_mb = self.select_iperf3_bw_from_flow()
                elif self.param.set_mb > 0:
                    self.param.set_mb %= self.param.flows
                else:
                    return RETURN_OK, client_cmd, server_cmd
                if DEBUG_MODE:
    ...
    def run(self, prefix=''):
        ...
        Public.off(self.param.src_host + self.param.dst_host)
        msg = ''
        set_flow = self.param.flows if self.param.flows else 'auto'
        msg = '%siperf3, src=%s, dst=%s, type=%s, flow=%s, set_kpps=%d, pktlen=%d' % \
              (prefix, self.param.src_host, self.param.dst_host, self.param.test_type, str(set_flow),
...
# def run_iperf3_task(tester, repeat=1):
#     """
#     函数功能：iperf3业务测试
#     :param tester: NetperfTester实例
#     :param repeat: 重复几次
