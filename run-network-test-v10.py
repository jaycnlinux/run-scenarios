#!/usr/bin/python
# -*- coding: utf-8 -*-
# __author__ = 'chengxiang'
# version: v0.8
# 修改内容：
# 20190421 增加TCP_CRR、qperf等N：N
# 脚本存在问题：
# 要加时间戳打印
# 同时加压和串行加压，在main函数里需要优化
# 为啥先sar，后netperf就出问题，netperf打了双倍的流，需要分析
# 切换成socket模式，避免scp方式数据拷贝


import sys
import getopt
import os
import time
import subprocess
import base64
import re
import copy
import logging
import gc
import math


# 全局变量 源IP、目标IP列表，流数
# 注意，传入的可能是EIP
SRC_HOST = '192.168.0.152'
DST_HOST = '192.168.0.101'
DEBUG_MODE = False
FLAVOR_TYPE = 'C3NE'
RATIO = 10
PASSWORD_HASH = '1234'
FLOW_STEP = {'1U': 16, '16U': 32, '32U': 64, '60U': 128}
RETURN_OK = 0
RETURN_FAIL = -1

ORDER_STEP = 0  # 依次测试，N:N的场景
ORDER_ALL = 1   # 同时测试，N:N的场景
BASE_DIR = 'log'
TASK_APP = ['iperf3_bg']
PRESS_ID = 'pps'


# ********** 内置数据库 **********
FLAVOR_BG_PRESS = {
    'C3NE': {
        '2U': {'KPPS': 400, 'BW': 1300},
        '4U': {'KPPS': 800, 'BW': 2500},
        '8U': {'KPPS': 1500, 'BW': 5000},
        '16U': {'KPPS': 2800, 'BW': 10000},
        '32U': {'KPPS': 5500, 'BW': 20000},
        '60U': {'KPPS': 10000, 'BW': 40000}
    },
    'C3': {
        '1U': {'KPPS': 150, 'BW': 300},
        '2U': {'KPPS': 300, 'BW': 600},
        '4U': {'KPPS': 500, 'BW': 1000},
        '8U': {'KPPS': 900, 'BW': 2000},
        '12U': {'KPPS': 1100, 'BW': 3000},
        '16U': {'KPPS': 1300, 'BW': 4000},
        '24U': {'KPPS': 2000, 'BW': 6000},
        '32U': {'KPPS': 2600, 'BW': 9000},
        '60U': {'KPPS': 5000, 'BW': 16000},
    },
    'M5': {
        '2U': {'KPPS': 60, 'BW': 400},
        '4U': {'KPPS': 125, 'BW': 750},
        '8U': {'KPPS': 250, 'BW': 1500},
        '16U': {'KPPS': 500, 'BW': 3000},
        '48U': {'KPPS': 1500, 'BW': 10000},
        '96U': {'KPPS': 3000, 'BW': 25000}
    },
    'C6': {
        '2U': {'KPPS': 400, 'BW': 1200},
        '4U': {'KPPS': 800, 'BW': 2400},
        '8U': {'KPPS': 1500, 'BW': 4500},
        '12U': {'KPPS': 2000, 'BW': 7000},
        '16U': {'KPPS': 2800, 'BW': 9000},
        '24U': {'KPPS': 4000, 'BW': 14000},
        '32U': {'KPPS': 5500, 'BW': 18000},
        '64U': {'KPPS': 10000, 'BW': 36000}
    }
}
# pwd
PASS_DICT = {'123': 'SHVhd2VpQDEyMw==',
             '1234': 'SHVhd2VpQDEyMzQ='}


class Logger(object):
    """ 日志对象 """
    def __init__(self, logfile='', prefix=''):
        if logfile:
            self.logfile = os.path.join(BASE_DIR, logfile)
        else:
            time_struct = time.localtime(time.time())
            logfile = time.strftime('%Y-%m-%d_%H%M%S.log', time_struct)
            if prefix:
                logfile = '%s-%s' % (prefix, logfile)
            else:
                logfile = '%s-%s' % ('real', logfile)
            self.logfile = os.path.join(BASE_DIR, logfile)
        # 创建log目录
        if not os.path.isdir(BASE_DIR):
            os.mkdir(BASE_DIR)
        # file logger
        self.logger_f = logging.getLogger(os.path.dirname(__file__))
        self.format_f = logging.Formatter('%(message)s')
        self.stream_f = logging.FileHandler(self.logfile)
        self.stream_f.setFormatter(self.format_f)
        self.logger_f.setLevel(logging.DEBUG)
        self.logger_f.addHandler(self.stream_f)
        # init file
        time_struct = time.localtime(time.time())
        msg = time.strftime('%Y-%m-%d_%H:%M:%S', time_struct)
        self.logger_f.info('======== LOG BEGIN %s ========' % msg)

    def set_logfile(self, logfile=None):
        """ 设置自定义文件名日志日志 """
        global BASE_DIR
        if not logfile:
            return
        if logfile:
            self.logfile = os.path.join(BASE_DIR, logfile)
        # remove old logfile
        self.logger_f.removeHandler(self.stream_f)
        del self.stream_f
        # new logfile
        if os.path.isfile(logfile):
            os.remove(logfile)
        self.logger_f = logging.getLogger(logfile)
        self.format_f = logging.Formatter('%(message)s')
        self.stream_f = logging.FileHandler(self.logfile)
        self.stream_f.setFormatter(self.format_f)
        self.logger_f.setLevel(logging.DEBUG)
        self.logger_f.addHandler(self.stream_f)
        # init loginfo
        time_struct = time.localtime(time.time())
        msg = time.strftime('%Y-%m-%d_%H:%M:%S', time_struct)
        self.logger_f.info('======== LOG BEGIN %s ========' % msg)

    def get_logfile(self):
        return self.logfile

    def crate_new_log(self, prefix=''):
        """ 重新生成当前时间的日志 """
        global BASE_DIR
        time_struct = time.localtime(time.time())
        logfile = time.strftime('%Y-%m-%d_%H%M%S.log', time_struct)
        if prefix:
            logfile = '%s-%s' % (prefix, logfile)
        else:
            logfile = '%s-%s' % ('real', logfile)
        self.logfile = os.path.join(BASE_DIR, logfile)
        self.set_logfile(self.logfile)

    def log(self, msg, print_screen=True):
        """ 打印日志 """
        # 局部变量
        format_color = {0: '%s',  # 默认色
                        1: '\033[32m%s\033[0m',  # 绿色
                        2: '\033[33m%s\033[0m',  # 黄色
                        3: '\033[31m%s\033[0m'  # 红色
                        }
        if not msg:
            return RETURN_FAIL

        self.logger_f.info(msg)
        if print_screen:
            print msg
        return RETURN_OK


class SleepTime(object):
    """    睡眠时钟对象    """
    milli_sec = 0.01
    one_sec = 1
    ten_sec = 2
    half_min = 3
    one_min = 6
    ten_min = 600
    half_hour = 1800

    def __init__(self):
        pass


def check_depends():
    """ 检查OB机器依赖 """
    # 检查expect命令
    cmd = 'expect -v'
    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if p.stderr.read():
        print '%s check failer, exit' % cmd
        return RETURN_FAIL
    p.stderr.close()
    print 'check_depends ok'
    return RETURN_OK


def parse_args():
    """
    函数功能：解析参数
    :return: NULL
    """
    global TASK_APP
    global NIC_NAME
    global OB_HOST
    global SRC_HOST
    global DST_HOST
    global FLAVOR_TYPE
    global RATIO
    global PRESS_ID
    global LOG_FILE
    # get nic name
    logger.log('check eth: begin')
    p = subprocess.Popen('ls /sys/class/net | grep -v lo', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    nic_list = p.stdout.readlines()
    p.stdout.close()
    # enumrate each nic
    for nic in nic_list:
        if re.search('eth\d+', nic) or re.search('ens\d+', nic):    # aws is ens
            NIC_NAME = nic.strip()
            break
    logger.log('found eth: %s' % NIC_NAME)
    # get local ip, and set to OB_HOST
    OB_HOST = Public.get_local_ip()
    logger.log('set ob=%s' % OB_HOST)
    # arguments parse
    if len(sys.argv) > 1:
        try:
            opts, args = getopt.getopt(sys.argv[1:], "hs:d:r:t:p:f:l:")
            logger.log(opts)
            for cmd, arg in opts:
                if cmd in '-s':
                    SRC_HOST = ''
                    with open(arg, 'r') as f:
                        for line in f:
                            SRC_HOST = SRC_HOST + line.strip() + ' '
                elif cmd in '-d':
                    DST_HOST = ''
                    with open(arg, 'r') as f:
                        for line in f:
                            DST_HOST = DST_HOST + line.strip() + ' '
                elif cmd in '-r':
                    RATIO = float(arg)
                elif cmd in '-t':
                    TASK_APP = arg
                elif cmd in '-p':
                    PRESS_ID = arg
                elif cmd in '-f':
                    FLAVOR_TYPE = arg
                elif cmd in '-l':
                    LOG_FILE = arg
                    if LOG_FILE != 'NULL':
                        logger.set_logfile(LOG_FILE)
                elif cmd in '-h':
                    Public.usage()
                    sys.exit(0)
        except Exception as e:
            logger.log('输入参数错误')
            logger.log(e)
            Public.usage()
            sys.exit(-1)


class SarType(object):
    rxpps = 'rxpps'
    txpps = 'txpps'
    rxbw = 'rxbw'
    txbw = 'txbw'
    all = 'all'

    def __init__(self):
        pass


class Public(object):
    def __init__(self):
        pass

    @staticmethod
    def create_new_log(prefix=''):
        """
        # 函数功能：创建日志文件
        :param prefix: 日志前缀
        """
        global LOG_FILE
        global BASE_DIR
        base_dir = BASE_DIR
        # 创建log目录
        if not os.path.isdir(base_dir):
            os.mkdir(base_dir)

        time_struct = time.localtime(time.time())
        log_suffix = time.strftime('%Y-%m-%d_%H%M%S.log', time_struct)
        if prefix:
            LOG_FILE = '%s%s%s_%s' % (base_dir, os.path.sep, prefix, log_suffix)
        else:
            LOG_FILE = '%s%s%s' % (base_dir, os.path.sep, log_suffix)

        with open(LOG_FILE, 'a+') as f:
            f.write('======== LOG BEGIN ========' + '\n')
        if DEBUG_MODE:
            logger.log('create_new_log: file=%s' % LOG_FILE)
        return LOG_FILE

    @staticmethod
    def set_ssh_trust(iplist=None, passidx='123'):
        """
        函数功能：设置ssh信任关系
        :param iplist: IP地址列表
        :param passidx: 密码索引
        """
        key_file = '/root/.ssh/id_rsa'
        password = base64.b64decode(PASS_DICT[passidx])
        ret_value = RETURN_OK
        # generate ssh key
        if not os.path.isfile(key_file):
            cmd = r"""
            expect -c '
            set timeout 10;
            spawn ssh-keygen -t rsa;
            expect {
                "*Enter file in which*" {
                    send "\r"
                    exp_continue
                }
                "*Enter passphrase*" {
                    send "\r"
                    exp_continue
                }
                "*Enter same passphrase again*" {
                    send "\r"
                    exp_continue
                }
                timeout  {
                    send_user "create ssh-key failed\n"
                    close
                }
            }
            '
            """
            p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            (output, error) = p.communicate()
            if DEBUG_MODE:
                print output
            time.sleep(SleepTime.one_sec)
        # copy-ssh-id
        for host in iplist:
            cmd = r"""
                expect -c '
                set timeout 10;
                spawn ssh-copy-id {host}
                expect {{
                    "*continue connecting*" {{
                        send "yes\r"
                        exp_continue
                    }}
                    "*assword*" {{
                        send "{password}\r"
                        exp_continue
                    }}
                    "*(yes/no)?*" {{
                        send "yes\r"
                        exp_continue
                    }}
                    "locked." {{
                        sleep 300
                        close
                        spawn ssh-copy-id -i /root/.ssh/id_rsa.pub root@$host
                        send_user "host {host} locked, retry in next 300 seconds"
                        exp_continue
                    }}
                    timeout  {{
                        send_user "ssh-copy-id to {host} failed\n"
                        close
                    }}
                }}
                '
            """.format(host=host, password="Pa123456!")
            # 注意，在原始字符串"""用format方法的时候，里面的{}被认为是符号变量，因此要用双重括号{{   }}去掉转义
            p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            (output, error) = p.communicate()
            # logger.log('ssh-copy-id %s result:' % host)
            if 'already installed' in output:
                # logger.log('ok')
                pass
            else:
                # logger.log('fail')
                pass

    @staticmethod
    def exec_shell_command(host='', cmd_list=None, backgroud=False):
        """
        # 函数功能：在指定主机上运行命令
        :param host: 所在主机，''=本地；返回值0=正常，非零=异常
        :param cmd_list: 命令
        :param backgroud: ssh的时候是否后台执行
        """
        # 局部变量
        ret = RETURN_OK
        tmp_cmd_file = 'tmp-cmd.sh'
        dst_cmd_file = 'run-cmd.sh'
        myip = Public.get_local_ip()

        if len(cmd_list) < 0:
            return RETURN_FAIL
        if DEBUG_MODE:
            logger.log('exec_shell_command: begin, host=%s cmd=%s' % (host, cmd_list))
        # 生成临时命令文件
        with open(tmp_cmd_file, 'w') as f:
            f.writelines([line + ' \n' for line in cmd_list])

        # 文件拷贝,并执行命令
        if host and host != myip:
            cmd = "script -q -c 'scp %s %s:/root/%s' >/dev/null" % (tmp_cmd_file, host, dst_cmd_file)
            ret = os.system(cmd)
            if backgroud:
                cmd = 'ssh %s sh /root/%s 2>/dev/null 1>/dev/null &' % (host, dst_cmd_file)
            else:
                cmd = 'ssh %s sh /root/%s 2>/dev/null 1>/dev/null' % (host, dst_cmd_file)
            ret = os.system(cmd)
        else:
            cmd = "script -q -c 'cp %s /root/%s' >/dev/null" % (tmp_cmd_file, dst_cmd_file)
            ret = os.system(cmd)
            cmd = 'sh /root/%s 2>/dev/null 1>/dev/null' % dst_cmd_file
            ret = os.system(cmd)

        # 移除临时文件
        os.remove(tmp_cmd_file)
        if DEBUG_MODE:
            logger.log('exec_shell_command: done')
        return ret

    @staticmethod
    def install_software(name_list, host_list):
        """
        # 函数功能：安装软件
        :param name_list: 待安装的软件列表
        :param host_list: 主机列表
        """
        tools_dir = 'tools'
        ret = ''

        soft_list = ['netperf', 'iperf3', 'qperf']
        s_list = list(set(soft_list) & set(name_list))
        ret = s_list
        if len(s_list) == 0:
            return ret

        for s in s_list:
            for host in host_list:
                if s == 'netperf':
                    # 不存在则安装
                    if Public.exec_shell_command('/usr/bin/netperf -V', host) != 0:
                        os.system("script -q -c 'scp %s/netperf %s:/usr/bin/' >/dev/null" % (tools_dir, host))
                        os.system("script -q -c 'scp %s/netserver %s:/usr/bin/' >/dev/null" % (tools_dir, host))
                        Public.exec_shell_command('chmod 777 /usr/bin/netperf', host)
                        Public.exec_shell_command('chmod 777 /usr/bin/netserver', host)
                elif s == 'iperf3':
                    if Public.exec_shell_command('/usr/bin/iperf3 -V', host) != 0:
                        os.system("script -q -c 'scp %s/iperf3 %s:/usr/bin/' >/dev/null" % (tools_dir, host))
                        os.system("script -q -c 'scp %s/libiperf.so.0 %s:/usr/local/lib/' >/dev/null" % (tools_dir, host))
                        Public.exec_shell_command('chmod 777 /usr/bin/iperf3', host)
                elif s == 'qperf':
                    if Public.exec_shell_command('/usr/bin/qperf -V', host) != 0:
                        os.system("script -q -c 'scp %s/qperf %s:/usr/bin/' >/dev/null" % (tools_dir, host))
                        Public.exec_shell_command('chmod 777 /usr/bin/qperf', host)

    @staticmethod
    def start_app_server(name, port_list=None, host_list=None):
        """
        # 函数功能: 启动server进程
        :param name: 程序名称
        :param port_list: 端口列表
        :param host_list: 主机列表
        """
        # 命令列表
        cmd_list = []

        if port_list is None:
            port_list = []
        if host_list is None:
            host_list = []

        name = str(name).lower()
        if name == 'netserver' or name == 'netperf':
            if len(port_list) == 0:
                cmd_list.append('netserver')
            else:
                for port in port_list:
                    cmd_list.append('netserver -p %d' % port)
        elif name == 'iperf3':
            if len(port_list) == 0:
                cmd_list.append('iperf3 -s')
            else:
                for port in port_list:
                    cmd_list.append('iperf3 -s -p %d' % port)
        elif name == 'qperf':
            cmd_list.append('qperf')

        if len(host_list) == 0:  # 本地执行
            for cmd in cmd_list:
                Public.exec_shell_command(cmd)
        else:  # 远程命令
            for host in host_list:
                for cmd in cmd_list:
                    Public.exec_shell_command(cmd, host)

    @staticmethod
    def off(host_list=None, app_list=None):
        """
        # 函数功能：关闭任务
        :param host_list: 主机列表，不写=local
        :param app_list: 程序列表，不写=all
        """
        # 脚本名
        script_name = sys.argv[0]
        # 应用程序列表
        app_list_template = ['netperf', 'netserver', 'iperf', 'iperf3', 'qperf', 'memcached', 'memaslap']
        tmp_cmd_file = 'tmp-cmd.sh'
        dst_cmd_file = 'run-cmd.sh'
        if app_list is None:
            app_list_new = app_list_template
        else:
            app_list_new = app_list

        if host_list is None:
            host_list = []

        if DEBUG_MODE:
            logger.log('off: host=%s app_list=%s' % (str(host_list), str(app_list_new)))
        app_name = '|'.join(app_list_new)
        cmd = "for i in `ps -ef | grep -v grep | grep -E '%s' | grep -v %s | awk '{print $2}'`;" \
              "do kill -9 $i;sleep 0.01;done" % (app_name, script_name)
        with open(tmp_cmd_file, 'w') as f:
            f.write(cmd + '\n')

        # 文件拷贝,并执行命令
        # 远程多主机
        if len(host_list) > 0:
            for host in host_list:
                ret = os.system("script -q -c 'scp %s %s:/root/%s' >/dev/null" % (tmp_cmd_file, host, dst_cmd_file))
                ret = os.system('ssh %s sh /root/%s 2>/dev/null 1>/dev/null' % (host, dst_cmd_file))
                time.sleep(SleepTime.milli_sec)
        # 本地执行
        else:
            ret = os.system("script -q -c 'cp %s /root/%s' >/dev/null" % (tmp_cmd_file, dst_cmd_file))
            ret = os.system('sh /root/%s 2>/dev/null 1>/dev/null' % dst_cmd_file)

        # 关闭服务后，等待一段时间
        time.sleep(SleepTime.one_sec)
        if os.path.isfile(tmp_cmd_file):
            os.remove(tmp_cmd_file)
        if DEBUG_MODE:
            logger.log('off: done')
        return RETURN_OK

    @staticmethod
    def get_vcpus_from_host(host=''):
        cpu_log = '/tmp/cpu.log'
        cpus = 1
        if host:
            cmd = ["lscpu | grep '^CPU(s)' | awk '{print $2}' > %s" % cpu_log]
            if DEBUG_MODE:
                logger.log('select_flow_from_host: host=%s' % host)
            Public.exec_shell_command(host, cmd)
            time.sleep(SleepTime.one_sec)
            os.system("script -q -c 'scp %s:%s %s' >/dev/null" % (host, cpu_log, cpu_log))
            # 如果文件不存在，返回默认流数
            if not os.path.isfile(cpu_log):
                return cpus
            with open(cpu_log, 'r') as f:
                try:
                    cpus = int(f.readline())
                except Exception as e:
                    cpus = 1
            # 清理临时文件
            if os.path.isfile(cpu_log):
                os.remove(cpu_log)
        if DEBUG_MODE:
            logger.log('get_vcpus_from_host: host=%s vcpu=%d' % (host, cpus) )
        return cpus

    @staticmethod
    def select_flow_from_host(host='', vcpu_ratio=0):
        """
        # 函数功能：根据主机CPU选择流数
        :param host: 要查询的的主机IP
        :param vcpu_ratio: 流数是CPU的几倍，如果为0则根据FLOW_DICT选择，>0直接返回 CPU数 * vcpu_ratio
        :return:
        """
        cpus = 1
        set_flow = 16
        if host:
            cpus = Public.get_vcpus_from_host(host)
            if vcpu_ratio > 0:
                # 流数是CPU的倍数
                set_flow = int(cpus * vcpu_ratio)
                if set_flow <= 0:
                    set_flow = 16
            else:
                # 根据cpu字典查找流数
                cpus_list = []
                for i in FLOW_STEP.keys():
                    cpus_list.append(int(i.replace('U', '')))
                # CPU字典从大到小排序
                cpus_list.sort(reverse=True)
                for i in cpus_list:
                    # 命中字典位置退出
                    if cpus >= i:
                        break
                # 从字典里取流数
                set_flow = FLOW_STEP['%dU' % i]
        if DEBUG_MODE:
            logger.log('select_flow_from_host: host=%s flow=%d ratio=%.2f' % (host, set_flow, float(vcpu_ratio)))
        return set_flow

    @staticmethod
    def select_kpps_from_flavor(host='', ratio=0):
        """
        根据flavor型号、加压%返回具体的kpps值
        :param host: 虚拟机
        :param ratio: 加压比例
        :return:
        """
        set_kpps = 0
        cpus = Public.get_vcpus_from_host(host)
        if FLAVOR_TYPE in FLAVOR_BG_PRESS.keys():
            flavor = '%dU' % cpus
            if flavor in FLAVOR_BG_PRESS[FLAVOR_TYPE].keys():
                set_kpps = FLAVOR_BG_PRESS[FLAVOR_TYPE][flavor]['KPPS']
                set_kpps = set_kpps * ratio
        return set_kpps

    @staticmethod
    def select_bw_from_flavor(host='', ratio=0):
        """
        根据flavor型号、加压%返回具体的bw值，单位M
        :param host: 虚拟机
        :param ratio: 加压比例
        :return:
        """
        set_bw = 0
        cpus = Public.get_vcpus_from_host(host)
        if FLAVOR_TYPE in FLAVOR_BG_PRESS.keys():
            flavor = '%dU' % cpus
            if flavor in FLAVOR_BG_PRESS[FLAVOR_TYPE].keys():
                set_bw = FLAVOR_BG_PRESS[FLAVOR_TYPE][flavor]['BW']
                set_bw = set_bw * ratio
        return set_bw

    @staticmethod
    def get_local_ip():
        ret_ip = ''
        p = subprocess.Popen('ifconfig %s' % NIC_NAME, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ret_str = p.stdout.read()
        p.stdout.close()
        ret_obj = re.search('inet\s\d+(?:\.\d+){3}', ret_str)
        if ret_obj:
            ret_ip = ret_obj.group().split()[-1]
        return ret_ip

    @staticmethod
    def bind_eip_to_vpc(host_list=None):
        global EIP_VPC_IP
        if len(host_list) == 0:
            return RETURN_FAIL
        for host in host_list:
            vpc_log = '/tmp/vpc.log'
            vpc_ip = ''
            cmd = ["ifconfig %s | grep 'inet ' | awk '{print $2}' > %s" % (NIC_NAME, vpc_log)]
            if DEBUG_MODE:
                logger.log('get eip from host %s' % host)
            Public.exec_shell_command(host, cmd)
            time.sleep(SleepTime.milli_sec)
            os.system("script -q -c 'scp %s:%s %s' >/dev/null" % (host, vpc_log, vpc_log))
            if os.path.isfile(vpc_log):
                with open(vpc_log, 'r') as f:
                    vpc_ip = f.readline().strip()
                # 清理临时文件
                os.remove(vpc_log)
            # 加入列表
            EIP_VPC_IP[host] = {'eip':host, 'vpc':vpc_ip}
        if DEBUG_MODE:
            logger.log('EIP_VPC_IP=%s' % str(EIP_VPC_IP))

    @staticmethod
    def usage():
        print '''Usage:sys.args[0] [option]
    -h：显示帮助信息
    -s: 源IP文件
    -d: 目标IP文件 
    -r: 加压比例[0,100]
    -t: 加压程序，iperf3_bg
    -p: 加压指标，pps还是bw
    -f: flavor，C3/C3NE
'''


class SarCollector(object):
    """" sar结果采集对象 """
    sar_send = 'send'
    sar_recv = 'recv'
    sar_both = 'both'
    send_sum = 'send_sum'
    recv_sum = 'recv_sum'
    header_format = '{:<12}  {:<5}  {:<11}  {:<11}  {:<11}  {:<11}'
    sar_format = '{:<12}  {:<5}  {:<11.2f}  {:<11.2f}  {:<11.2f}  {:<11.2f}'

    def __init__(self, src_host=None, dst_host=None, c_time=60, c_type='all',
                 direction='send', details=False, eth='', sleep_time=30):
        self.src_host = copy.deepcopy(src_host) if src_host else []
        self.dst_host = copy.deepcopy(dst_host) if dst_host else []
        self.c_time = c_time
        self.c_type = c_type
        self.direction = direction
        self.details = details
        self.eth = eth if eth else NIC_NAME
        self.sleep_time = sleep_time
        self.send_sum = 'send_sum'
        self.recv_sum = 'recv_sum'
        self.data = {}
        self.init_data()
        self.summary = {}
        self.log_format = '/root/%s_sar.log'

    def init_data(self):
        """ 初始化sar结果 """
        if self.data:
            del self.data
            gc.collect()
        self.data = {self.send_sum: {'sar': [], 'avg': []},
                     self.recv_sum: {'sar': [], 'avg': []}
                     }
        if self.src_host:
            for i in self.src_host:
                self.data[i] = {'sar': [], 'avg': []}
        if self.dst_host:
            for i in self.dst_host:
                self.data[i] = {'sar': [], 'avg': []}
        # 多次测试的结果

    def set_param(self, src_host=None, dst_host=None, c_time=None, c_type=None, direction=None, details=None, eth='eth0'):
        """ 设置sar采集参数 """
        self.src_host = copy.deepcopy(src_host)
        self.dst_host = copy.deepcopy(dst_host)
        if c_time:
            self.c_time = c_time
        if c_type:
            self.c_type = c_type
        if direction:
            self.direction = direction
        if details:
            self.details = details
        if eth:
            self.eth = eth

    def __save_data(self, host, sar_file=None):
        """ 保存一个host的sar """
        if not host or not sar_file:
            logger.log('ERROR: host or sar_file is None')
            return RETURN_FAIL
        # 读取sar文件
        if not os.path.isfile(sar_file):
            return RETURN_FAIL
        with open(sar_file, 'r') as f:
            for line in f:
                if ('Average' in line) and (self.eth in line):  # average
                    ret_list = line.strip().split()
                    ele = [
                           ret_list[0],     # average
                           ret_list[1],     # eth
                           round(float(ret_list[2]), 2),  # rxpps
                           round(float(ret_list[3]), 2),  # txpps
                           round(float(ret_list[4]), 2),  # rxbw
                           round(float(ret_list[5]), 2)]  # txbw
                    if host in self.data.keys():
                        self.data[host]['avg'] = ele      # 修改记录
                    else:
                        self.data[host] = {'sar': [], 'avg': ele}  # 不存在则增加记录
                elif self.eth in line:      # 每秒值
                    ret_list = line.strip().split()
                    ele = [
                           ret_list[0] + ' ' + ret_list[1],  # time
                           ret_list[2],                   # eth
                           round(float(ret_list[3]), 2),  # rxpps
                           round(float(ret_list[4]), 2),  # txpps
                           round(float(ret_list[5]), 2),  # rxbw
                           round(float(ret_list[6]), 2)]  # txbw
                    if host in self.data.keys():
                        self.data[host]['sar'].append(ele)  # 修改记录
                    else:
                        self.data[host] = {'sar': [ele], 'avg': []}  # 不存在则增加记录
        # 删除临时文件
        time.sleep(3)
        # logger.log('remove %s' % sar_file)
        if os.path.isfile(sar_file):
            os.remove(sar_file)
        return RETURN_OK

    def __calc_sum(self, host_list, sum_type='send_sum'):
        """ 计算所有host的sar平均值 """
        if not host_list:
            return RETURN_FAIL

        sum_rx_pps = 0
        sum_tx_pps = 0
        sum_rx_bw = 0
        sum_tx_bw = 0
        for i in range(0, len(self.data[host_list[0]]['sar'])):
            rx_pps = 0
            tx_pps = 0
            rx_bw = 0
            tx_bw = 0
            # 所有主机带宽相加
            for host in host_list:
                rx_pps += self.data[host]['sar'][i][2]
                tx_pps += self.data[host]['sar'][i][3]
                rx_bw += self.data[host]['sar'][i][4]
                tx_bw += self.data[host]['sar'][i][5]
            # 所有主机一行的和
            sum_rx_pps += rx_pps
            sum_tx_pps += tx_pps
            sum_rx_bw += rx_bw
            sum_tx_bw += tx_bw
            self.data[sum_type]['sar'].append(['%ds' % (i+1), self.eth, rx_pps, tx_pps, rx_bw, tx_bw])
        # 所有人的平均
        sum_rx_pps = sum_rx_pps/len(self.data[host_list[0]]['sar'])
        sum_tx_pps = sum_tx_pps/len(self.data[host_list[0]]['sar'])
        sum_rx_bw = sum_rx_bw/len(self.data[host_list[0]]['sar'])
        sum_tx_bw = sum_tx_bw/len(self.data[host_list[0]]['sar'])
        self.data[sum_type]['avg'] = [self.data[host_list[0]]['avg'][0],
                                      self.data[host_list[0]]['avg'][1],
                                      sum_rx_pps, sum_tx_pps, sum_rx_bw, sum_tx_bw
                                      ]

    def get_sar_data(self, host_list, sum_type, prefix=''):
        for i in host_list:
            sar_log = self.log_format % i
            os.system("script -q -c 'scp %s:%s %s' >/dev/null" % (i, sar_log, sar_log))
            if DEBUG_MODE:
                logger.log('host:%s\tsar -n DEV 1 %d' % (i, self.c_time))
            # 打印sar采集标记
            if host_list == self.src_host:
                direction = 'send'
            else:
                direction = 'recv'
            logger.log('sar on %s: %s' % (direction, i))
            self.__save_data(i, sar_log)
            # 打印数据记录
            # 表头
            msg = self.header_format.format('00:00:00 AM', 'IFACE', 'rxpck/s', 'txpck/s', 'rxkB/s', 'txkB/s')
            logger.log(msg)
            # host明细数据
            for record in self.data[i]['sar']:
                msg = self.sar_format.format(record[0], record[1], record[2], record[3], record[4], record[5])
                logger.log(msg)
            # host average
            msg = self.sar_format.format(self.data[i]['avg'][0],
                                         self.data[i]['avg'][1],
                                         self.data[i]['avg'][2],
                                         self.data[i]['avg'][3],
                                         self.data[i]['avg'][4],
                                         self.data[i]['avg'][5],
                                         )
            logger.log(msg)
        # 所有主机汇总
        self.__calc_sum(host_list, sum_type)
        # 仅当并行打流ECS数量大于1才打印
        if len(host_list) > 1:
            logger.log('sar %s on %s' % (sum_type, str(host_list)))
            # 表头
            msg = self.header_format.format('00:00:00 AM', 'IFACE', 'rxpck/s', 'txpck/s', 'rxkB/s', 'txkB/s')
            logger.log(msg)
            for record in self.data[sum_type]['sar']:
                msg = self.sar_format.format(record[0], record[1], record[2], record[3], record[4], record[5])
                logger.log(msg)
            # 平均
            record = self.data[sum_type]['avg']
            msg = self.sar_format.format(record[0], record[1], record[2], record[3], record[4], record[5])
            logger.log(msg)

    def copy_data(self, send_sum=None, recv_sum=None):
        """ 拷贝汇总数据 """
        if send_sum and self.src_host:
            send_sum['eth'] = self.data['send_sum']['sar'][0][1]
            for record in self.data['send_sum']['sar']:
                send_sum['rxpps'].append(record[2])
                send_sum['txpps'].append(record[3])
                send_sum['rxbw'].append(record[4])
                send_sum['txbw'].append(record[5])
        if recv_sum and self.dst_host:
            recv_sum['eth'] = self.data['recv_sum']['sar'][0][1]
            for record in self.data['recv_sum']['sar']:
                recv_sum['rxpps'].append(record[2])
                recv_sum['txpps'].append(record[3])
                recv_sum['rxbw'].append(record[4])
                recv_sum['txbw'].append(record[5])

    def generate_percentile(self, percent=(1, 5, 10)):
        """ 生成百分比，默认比较1%、5%、10%、90%、95%、99%
        """
        def percentile(data_list, percent):
            if percent < 0.000001:      # 浮点型，不能用==
                return min(data_list)
            elif percent > 0.999999:
                return max(data_list)

            count = len(data_list)
            sum = (count - 1) * percent  # 计算
            pos_i = int(math.floor(sum))
            pos_j = sum - pos_i
            # print 'sum=%f\tpos_i=%d\tpos_j=%f' % (sum, pos_i, pos_j)
            percent_data = (1 - pos_j) * data_list[pos_i] + pos_j * data_list[pos_i + 1]
            return percent_data

            # 计算平均值
            sum = 0
            for i in data_list:
                sum += i
            qos0 = sum / len(data_list)

            # 计算百分位
            qos1_t = get_data(data_list, percent)
            qos2_t = get_data(data_list, 1 - percent)
            if qos1_t < qos2_t:
                qos1 = qos1_t
                qos2 = qos2_t
            else:
                qos1 = qos2_t
                qos2 = qos1_t

            return float('%.2f' % qos0), float('%.2f' % qos1), float('%.2f' % qos2), \
                   float('%.2f' % (100 - qos1 * 100. / qos0)), float('%.2f' % (qos2 * 100. / qos0 - 100))
        pass

    def fetch_data(self):
        # 等待采集结束
        if DEBUG_MODE:
            logger.log('sar: begin fetch_data')
        time.sleep(self.c_time + self.sleep_time + SleepTime.ten_sec)
        # 获取数据
        if self.src_host:
            self.get_sar_data(self.src_host, self.send_sum)
        if self.dst_host:
            self.get_sar_data(self.dst_host, self.recv_sum)
        # 结束
        if DEBUG_MODE:
            logger.log('%s end' % self.__class__.__name__)

    def run(self, sleep_time=30):
        """ sar采集入口
        :param sleep_time: 等待时间，这样可以让sar指令提前运行，避免打流后发送sar指令拥塞
        """
        # 等待时间
        if sleep_time:
            self.sleep_time = sleep_time
        # 初始化数据
        self.init_data()
        if DEBUG_MODE:
            logger.log('%s: begin' % repr(self.__class__.__name__))

        # 参数校验
        if (self.src_host is None) and (self.dst_host is None):
            logger.log('ERROR: src_host and dst_host is None')
            return RETURN_FAIL
        if self.c_type and (self.c_type not in [SarType.rxpps, SarType.txpps, SarType.rxbw, SarType.txbw, SarType.all]):
            logger.log('ERROR: sar_type error')
            return RETURN_FAIL
        if DEBUG_MODE:
            logger.log('sar param:%s' % repr(self.__dict__))
        # 为了保障采集时间一致，不做sleep等待
        if self.src_host:
            for i in self.src_host:
                sar_log = self.log_format % i
                if sleep_time:
                    cmd = 'sleep %d;sar -n DEV 1 %d > %s &' % (sleep_time, self.c_time, sar_log)
                else:
                    cmd = 'sar -n DEV 1 %d > %s &' % (self.c_time, sar_log)
                Public.exec_shell_command(i, [cmd], backgroud=True)     # 后台执行，不要阻塞
        if self.dst_host:
            for i in self.dst_host:
                sar_log = self.log_format % i
                if sleep_time:
                    cmd = 'sleep %d;sar -n DEV 1 %d > %s &' % (sleep_time, self.c_time, sar_log)
                else:
                    cmd = 'sar -n DEV 1 %d > %s &' % (self.c_time, sar_log)
                Public.exec_shell_command(i, [cmd], backgroud=True)     # 后台执行，不要阻塞

        return RETURN_OK


class TestParam(object):
    """    测试参数类    """
    def __init__(self, src_host=None, dst_host=None, test_type='udp_stream', base_port=7001, flows=16, flow_cpu_ratio=0.0,
                 set_mb=-1, set_kpps=-1, test_time=3600, pkt_len=64, per_host_kpps_ratio=-1, per_host_bw_ratio=-1):
        """
        # 测试参数
        :param src_host: 源IP列表
        :param dst_host: 目的IP列表
        :param test_type: 测试类型tcp_stream, iperf3
        :param base_port: 起始端口
        :param flows: 流数
        :param flow_cpu_ratio: 流数是CPU的几倍，优先级高，如果设置了，则忽略flows的输入
        :param set_mb: 设定的总带宽，仅iperf3有效
        :param set_kpps: 设定的PPS，单位K，仅iperf3有效，-1表示未指定
        :param test_time: 测试时长
        :param pkt_len: 包长
        :param per_host_kpps_ratio: 每个ECS pps加压比例
        :param per_host_bw_ratio: 每个ECS带宽加压比例
        """
        if src_host:
            self.src_host = copy.deepcopy(src_host)
        else:
            self.src_host = []
        if dst_host:
            self.dst_host = copy.deepcopy(dst_host)
        else:
            self.dst_host = []
        self.test_type = test_type
        self.base_port = base_port
        self.flows = flows
        self.flow_cpu_ratio = flow_cpu_ratio
        self.set_mb = set_mb
        self.flow_mb = set_mb
        self.set_kpps = set_kpps
        self.test_time = test_time
        self.pkt_len = pkt_len if pkt_len > 0 else 64
        self.per_host_kpps_ratio = per_host_kpps_ratio
        self.per_host_bw_ratio = per_host_bw_ratio

    def set_param(self, src_host=None, dst_host=None, test_type=None, base_port=None, flows=None, flow_cpu_ratio=None,
                  set_mb=None, set_kpps=None, test_time=None, pkt_len=None, per_host_kpps_ratio=None, per_host_bw_ratio=None):
        """
        # 测试参数
        :param src_host: 源IP列表
        :param dst_host: 目的IP列表
        :param test_type: 测试类型tcp_stream, iperf3
        :param base_port: 起始端口
        :param flows: 流数
        :param set_mb: 设定的带宽，仅iperf3有效，优先级低
        :param set_kpps: 设定的PPS，单位K，仅iperf3有效，优先级低
        :param test_time: 测试时长
        :param pkt_len: 包长
        :param per_host_kpps_ratio: 每个ECS pps加压比例，优先级高
        :param per_host_bw_ratio: 每个ECS带宽加压比例，优先级高
        """
        if src_host:
            if type(src_host) is list:
                self.src_host = copy.deepcopy(src_host)
            else:
                self.src_host = copy.deepcopy([src_host])
        if dst_host:
            if type(dst_host) is list:
                self.dst_host = copy.deepcopy(dst_host)
            else:
                self.dst_host = copy.deepcopy([dst_host])
        if test_type:
            self.test_type = test_type
        if base_port:
            self.base_port = base_port
        if flows:
            self.flows = flows
        if flow_cpu_ratio:
            self.flow_cpu_ratio = flow_cpu_ratio
        if set_mb:
            self.set_mb = set_mb
            self.flow_mb = set_mb
        if set_kpps:
            self.set_kpps = set_kpps
        if test_time:
            self.test_time = test_time
        if pkt_len:
            self.pkt_len = pkt_len if pkt_len > 0 else 64
        self.per_host_kpps_ratio = per_host_kpps_ratio
        self.per_host_bw_ratio = per_host_bw_ratio

    @property
    def check_param(self):
        """
        参数有效性校验
        """
        ret = RETURN_OK
        if (self.base_port <= 0) or (self.flows <= 0) or (self.test_time <= 0) or (self.pkt_len <= 0):
            ret = RETURN_FAIL
        return ret


class NetperfTester(object):
    def __init__(self, param=None, sar=None):
        self.param = copy.deepcopy(param) if param else TestParam()
        self.sar = copy.deepcopy(sar) if sar else SarCollector()
        self.log_format = '/root/%s_netperf_%d.log'
        # 保存所有打流的host，port信息，因为拉取远端日志需要，格式{'192.168.0.1':['/root/xxx.log'],xxx}
        self.log_list = {}

    def make_netperf_cmd(self, src_host, dst_host):
        """
        # 函数功能：单向netperf测试命令解析
        """
        # 局部变量：返回结果值，列表成员为字典格式{host='192.168.1.1', cmd = [cmd1,cmd2,cmd3...]}
        client_cmd = []
        server_cmd = []
        auto_flow = self.param.flows
        test_method_template = ['TCP_STREAM', 'UDP_STREAM', 'TCP_RR', 'UDP_RR', 'TCP_CRR']

        test_type = self.param.test_type.upper()
        if test_type not in test_method_template:
            return RETURN_FAIL, client_cmd, server_cmd
        if len(src_host) == 0 or len(dst_host) == 0:
            return RETURN_FAIL, client_cmd, server_cmd

        port_index = self.param.base_port
        # 初始命令字典
        for i in src_host:
            element = dict(host=i, cmd=[])
            client_cmd.append(element)
        for i in dst_host:
            element = dict(host=i, cmd=[])
            server_cmd.append(element)

        if DEBUG_MODE:
            logger.log('make_netperf_cmd begin')
        # 只适用于1:N  N:1 或者N:N的场景，不支持N:M的复杂场景。
        netperf_method = '-m'
        if self.param.test_type.upper() in ['TCP_CRR', 'TCP_RR', 'UDP_RR']:
            netperf_method = '-r'
        # N:N
        if len(src_host) == len(dst_host):
            for client_eip,server_eip in zip(src_host, dst_host):
                # 传入flows=0则表示自动判断流数
                if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                    ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
                    auto_flow = Public.select_flow_from_host(client_eip, vcpu_ratio=ratio)
                    logger.log('host=%s flows=%d' % (client_eip, auto_flow))
                for j in range(0, auto_flow):
                    # 转换VPC_IP
                    server_vpc = EIP_VPC_IP[server_eip]['vpc'] if EIP_VPC_IP[server_eip]['vpc'] else server_eip
                    netperf_log = self.log_format % (client_eip, port_index)
                    if client_eip not in self.log_list.keys(): self.log_list[client_eip] = []
                    self.log_list[client_eip].append(netperf_log)
                    c_cmd = 'netperf -H %s -p %d -t %s -l %d -- %s %d > %s &' \
                            % (server_vpc, port_index, test_type, self.param.test_time, netperf_method, self.param.pkt_len, netperf_log)
                    s_cmd = 'netserver -p %d 2>/dev/null 1>/dev/null &' % port_index
                    port_index += 1
                    # 记录命令
                    for ele in client_cmd:
                        if ele['host'] == client_eip:
                            ele['cmd'].append(c_cmd)
                            break
                    for ele in server_cmd:
                        if ele['host'] == server_eip:
                            ele['cmd'].append(s_cmd)
                            break
        # 1:N
        elif len(src_host) == 1 and len(dst_host) > 1:
            # 传入flows=0则表示自动判断流数
            if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
                auto_flow = Public.select_flow_from_host(src_host[0], vcpu_ratio=ratio)
                logger.log('host=%s flows=%d' % (src_host[0], auto_flow))
            per_flow = auto_flow / len(dst_host)
            left_flow = auto_flow % len(dst_host)
            for server_eip in dst_host:
                client_eip = src_host[0]
                # 转换VPC_IP
                server_vpc = EIP_VPC_IP[server_eip]['vpc'] if EIP_VPC_IP[server_eip]['vpc'] else server_eip
                netperf_log = self.log_format % (client_eip, port_index)
                if client_eip not in self.log_list.keys(): self.log_list[client_eip] = []
                self.log_list[client_eip].append(netperf_log)
                for j in range(0, per_flow):
                    c_cmd = 'netperf -H %s -p %d -t %s -l %d -- %s %d > %s &' \
                            % (server_vpc, port_index, test_type, self.param.test_time, netperf_method, self.param.pkt_len, netperf_log)
                    s_cmd = 'netserver -p %d 2>/dev/null 1>/dev/null &' % port_index
                    port_index += 1
                    # 记录命令
                    for ele in client_cmd:
                        if ele['host'] == src_host[0]:
                            ele['cmd'].append(c_cmd)
                            break
                    for ele in server_cmd:
                        if ele['host'] == server_eip:
                            ele['cmd'].append(s_cmd)
                            break

            for j in range(0, left_flow):
                # 转换VPC_IP
                client_eip = src_host[0]
                server_eip = dst_host[-1]
                server_vpc = EIP_VPC_IP[server_eip]['vpc'] if EIP_VPC_IP[server_eip]['vpc'] else server_eip
                netperf_log = self.log_format % (client_eip, port_index)
                if client_eip not in self.log_list.keys(): self.log_list[client_eip] = []
                self.log_list[client_eip].append(netperf_log)
                c_cmd = 'netperf -H %s -p %d -t %s -l %d -- %s %d > %s &' \
                        % (server_vpc, port_index, test_type, self.param.test_time, netperf_method, self.param.pkt_len, netperf_log)
                s_cmd = 'netserver -p %d 2>/dev/null 1>/dev/null &' % port_index
                port_index += 1
                # 记录命令
                for ele in client_cmd:
                    if ele['host'] == src_host[0]:
                        ele['cmd'].append(c_cmd)
                        break
                for ele in server_cmd:
                    if ele['host'] == dst_host[-1]:
                        ele['cmd'].append(s_cmd)
                        break
        # N:1
        elif len(src_host) > 1 and len(dst_host) == 1:
            if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
                auto_flow = Public.select_flow_from_host(dst_host[0], vcpu_ratio=ratio)
                logger.log('host=%s flows=%d' % (dst_host[0], auto_flow))
            per_flow = auto_flow / len(src_host)
            left_flow = auto_flow % len(src_host)
            for client_eip in src_host:
                # 转换VPC_IP
                server_eip = dst_host[0]
                server_vpc = EIP_VPC_IP[server_eip]['vpc'] if EIP_VPC_IP[server_eip]['vpc'] else server_eip
                netperf_log = self.log_format % (client_eip, port_index)
                if client_eip not in self.log_list.keys(): self.log_list[client_eip] = []
                self.log_list[client_eip].append(netperf_log)
                for j in range(0, per_flow):
                    c_cmd = 'netperf -H %s -p %d -t %s -l %d -- %s %d > %s &' \
                            % (server_vpc, port_index, test_type, self.param.test_time, netperf_method, self.param.pkt_len, netperf_log)
                    s_cmd = 'netserver -p %d 2>/dev/null 1>/dev/null &' % port_index
                    port_index += 1
                    # 记录命令
                    for ele in client_cmd:
                        if ele['host'] == client_eip:
                            ele['cmd'].append(c_cmd)
                            break
                    for ele in server_cmd:
                        if ele['host'] == server_eip:
                            ele['cmd'].append(s_cmd)
                            break

            for j in range(0, left_flow):
                # 转换VPC_IP
                client_eip = src_host[-1]
                server_eip = dst_host[-1]
                server_vpc = EIP_VPC_IP[server_eip]['vpc'] if EIP_VPC_IP[server_eip]['vpc'] else server_eip
                netperf_log = self.log_format % (client_eip, port_index)
                if client_eip not in self.log_list.keys(): self.log_list[client_eip] = []
                self.log_list[client_eip].append(netperf_log)
                c_cmd = 'netperf -H %s -p %d -t %s -l %d -- %s %d > %s &' \
                        % (server_vpc, port_index, test_type, self.param.test_time, netperf_method, self.param.pkt_len, netperf_log)
                s_cmd = 'netserver -p %d 2>/dev/null 1>/dev/null &' % port_index
                port_index += 1
                # 记录命令
                for ele in client_cmd:
                    if ele['host'] == src_host[-1]:
                        ele['cmd'].append(c_cmd)
                        break
                for ele in server_cmd:
                    if ele['host'] == dst_host[0]:
                        ele['cmd'].append(s_cmd)
                        break
        # N:M复杂场景，不支持
        else:
            return RETURN_FAIL, client_cmd, server_cmd

        if DEBUG_MODE:
            logger.log('make_netperf_cmd end')
        return RETURN_OK, client_cmd, server_cmd

    def init_data(self):
        self.log_list = {}

    def stop(self):
        """ 停止测试 """
        Public.off(self.param.src_host + self.param.dst_host, ['netperf', 'netserver'])

    def fetch_data(self):
        logger.log('netperf: fetch_data begin')
        for host,logs in self.log_list.items():
            for each_log in logs:
                cmd = "script -q -c 'scp %s:%s %s' >/dev/null" % (host, each_log, each_log)
                os.system(cmd)
                time.sleep(SleepTime.milli_sec)
        if DEBUG_MODE:
            logger.log('netperf: fetch_data finish')

    def run(self):
        """    功能，运行netperf命令    """
        self.init_data()
        # 停止以前的流
        self.stop()
        r, c, s = self.make_netperf_cmd(self.param.src_host, self.param.dst_host)
        if DEBUG_MODE:
            logger.log('NetperfTester run: src=%s  dst=%s' % (str(self.param.src_host), str(self.param.dst_host)))
            logger.log('test.param=%s' % repr(self.param.__dict__))
        if r == RETURN_FAIL:
            return RETURN_FAIL
        if DEBUG_MODE:
            logger.log('call_netperf: start server')
        # 启动server
        for i in s:
            Public.exec_shell_command(i['host'], i['cmd'])
            time.sleep(SleepTime.milli_sec)
        time.sleep(SleepTime.ten_sec)
        # 启动client
        if DEBUG_MODE:
            logger.log('call_netperf: start client')
        for i in c:
            Public.exec_shell_command(i['host'], i['cmd'])
            time.sleep(SleepTime.milli_sec)
        if DEBUG_MODE:
            logger.log(self.log_list)
            logger.log('NetperfTester run: done')


class Iperf3Tester(object):
    def __init__(self, param=None, sar=None):
        self.param = copy.deepcopy(param) if param else TestParam()
        self.sar = copy.deepcopy(sar) if sar else SarCollector()

    def select_iperf3_bw_from_flow_and_pps(self):
        """
        # 函数功能：根据流数和包长计算iperf3 带宽
        """
        ret_bw = self.param.set_mb
        if (self.param.set_kpps > 0) and (self.param.flows > 0) and (self.param.pkt_len > 0):
            ret_bw = float(self.param.set_kpps*1000.0*self.param.pkt_len*8/self.param.flows/1000/1000)
        self.param.flow_mb = ret_bw
        return ret_bw

    def make_iperf3_cmd(self):
        """
        # 函数功能：单向iperf3测试命令解析
        """
        # 局部变量：返回结果值，列表成员为字典格式{host='192.168.1.1', cmd = [cmd1,cmd2,cmd3...]}
        client_cmd = []
        server_cmd = []
        test_method_template = ['TCP', 'UDP']

        test_type = self.param.test_type.upper()
        if test_type not in test_method_template:
            logger.log('test_type not int %s' % str(test_method_template))
            return RETURN_FAIL, client_cmd, server_cmd
        if len(self.param.src_host) == 0 or len(self.param.dst_host) == 0:
            logger.log('src_host = 0 or dst_host = 0')
            return RETURN_FAIL, client_cmd, server_cmd

        port_index = self.param.base_port
        # 初始命令字典
        for i in self.param.src_host:
            element = dict(host=i, cmd=[])
            client_cmd.append(element)
        for i in self.param.dst_host:
            element = dict(host=i, cmd=[])
            server_cmd.append(element)

        # 只适用于1:N N:1 或者N:N的场景，不支持N:M的复杂场景。
        # N:N
        if len(self.param.src_host) == len(self.param.dst_host):
            # 传入flows=0则表示自动判断流数，每个src_host都要判断
            for i in range(0, len(self.param.src_host)):
                if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                    ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
                    self.param.flows = Public.select_flow_from_host(self.param.src_host[i], ratio)
                    logger.log('host=%s flows=%d kpps=%d' % (self.param.src_host[i], self.param.flows, self.param.set_kpps))
                # 是否设定了加压比例
                if self.param.per_host_kpps_ratio > 0:
                    self.param.set_kpps = Public.select_kpps_from_flavor(self.param.src_host[i],
                                                                         self.param.per_host_kpps_ratio)
                elif self.param.per_host_bw_ratio > 0:
                    self.param.set_mb = Public.select_bw_from_flavor(self.param.src_host[i],
                                                                     self.param.per_host_bw_ratio)
                # 然后设置-b参数
                if self.param.set_kpps > 0:
                    self.param.flow_mb = self.select_iperf3_bw_from_flow_and_pps()
                elif self.param.set_mb > 0:
                    self.param.flow_mb = self.param.set_mb*1.0/self.param.flows
                else:
                    # PPS和BW都<=0，说明pps=0或者bw=0，不需要加压
                    logger.log('no need to press')
                    return RETURN_OK, client_cmd, server_cmd
                if DEBUG_MODE:
                    logger.log('make_iperf3_cmd: self.param=%s' % repr(self.param.__dict__))
                # 转换VPC_IP
                host_eip = self.param.dst_host[i]
                host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
                for j in range(0, self.param.flows):
                    if test_type == 'TCP':
                        # TCP不需要设置iperf3 -l参数
                        c_cmd = 'iperf3 -c %s -p %d -b %.3fM -t %d 2>/dev/null 1>/dev/null &' \
                                % (host_vpc, port_index, float(self.param.flow_mb), self.param.test_time)
                    else:
                        c_cmd = 'iperf3 -c %s -p %d -u -b %.3fM -t %d -l %d 2>/dev/null 1>/dev/null &' \
                                % (host_vpc, port_index, float(self.param.flow_mb),
                                   self.param.test_time, self.param.pkt_len)

                    s_cmd = 'iperf3 -s -p %d 2>/dev/null 1>/dev/null &' % port_index
                    port_index += 1
                    # 记录命令
                    for ele in client_cmd:
                        if ele['host'] == self.param.src_host[i]:
                            ele['cmd'].append(c_cmd)
                            break
                    for ele in server_cmd:
                        if ele['host'] == self.param.dst_host[i]:
                            ele['cmd'].append(s_cmd)
                            break
        # 1:N
        elif len(self.param.src_host) == 1 and len(self.param.dst_host) > 1:
            # 传入flows=0则表示自动判断流数
            ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
            if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                self.param.flows = Public.select_flow_from_host(self.param.src_host[0], ratio)
                logger.log('host=%s flows=%d kpps=%d' % (self.param.src_host[0], self.param.flows, self.param.set_kpps))
            # 是否设定了加压比例
            if self.param.per_host_kpps_ratio > 0:
                self.param.set_kpps = Public.select_kpps_from_flavor(self.param.src_host[0],
                                                                     self.param.per_host_kpps_ratio)
            elif self.param.per_host_bw_ratio > 0:
                self.param.set_mb = Public.select_bw_from_flavor(self.param.src_host[0], self.param.per_host_bw_ratio)
            # set set_mb according to kpps
            if self.param.set_kpps > 0:
                self.param.flow_mb = self.select_iperf3_bw_from_flow_and_pps()
            elif self.param.set_mb > 0:
                self.param.flow_mb = self.param.set_mb*1.0/self.param.flows
            else:
                # PPS和BW都<=0，说明pps=0或者bw=0，不需要加压
                logger.log('no need to press')
                return RETURN_OK, client_cmd, server_cmd
            if DEBUG_MODE:
                logger.log('make_iperf3_cmd: self.param=%s' % repr(self.param.__dict__))
            per_flow = self.param.flows / len(self.param.dst_host)
            left_flow = self.param.flows % len(self.param.dst_host)
            for i in range(0, len(self.param.dst_host)):
                # 转换VPC_IP
                host_eip = self.param.dst_host[i]
                host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
                for j in range(0, per_flow):
                    if test_type == 'TCP':
                        c_cmd = "iperf3 -c %s -p %d -b %.3fM -t %d 2>/dev/null 1>/dev/null &" \
                                % (host_vpc, port_index, float(self.param.flow_mb), self.param.test_time)
                    else:
                        c_cmd = "iperf3 -c %s -p %d -u -b %.3fM -t %d -l %d 2>/dev/null 1>/dev/null &" \
                                % (host_vpc, port_index, float(self.param.flow_mb),
                                   self.param.test_time, self.param.pkt_len)

                    s_cmd = 'iperf3 -s -p %d 2>/dev/null 1>/dev/null &' % port_index
                    port_index += 1
                    # 记录命令
                    for ele in client_cmd:
                        if ele['host'] == self.param.src_host[0]:
                            ele['cmd'].append(c_cmd)
                            break
                    for ele in server_cmd:
                        if ele['host'] == self.param.dst_host[i]:
                            ele['cmd'].append(s_cmd)
                            break
            for j in range(0, left_flow):
                # 转换VPC_IP
                host_eip = self.param.dst_host[-1]
                host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
                if test_type == 'TCP':
                    c_cmd = "iperf3 -c %s -p %d -b %.3fM -t %d 2>/dev/null 1>/dev/null &"\
                            % (host_vpc, port_index, float(self.param.flow_mb), self.param.test_time)
                # UDP
                else:
                    c_cmd = "iperf3 -c %s -p %d -u -b %.3fM -t %d -l %d 2>/dev/null 1>/dev/null &" \
                            % (host_vpc, port_index, float(self.param.flow_mb),
                               self.param.test_time, self.param.pkt_len)

                s_cmd = 'iperf3 -s -p %d 2>/dev/null 1>/dev/null &' % port_index
                port_index += 1
                # 记录命令
                for ele in client_cmd:
                    if ele['host'] == self.param.src_host[0]:
                        ele['cmd'].append(c_cmd)
                        break
                for ele in server_cmd:
                    if ele['host'] == self.param.dst_host[-1]:
                        ele['cmd'].append(s_cmd)
                        break
        # N:1
        elif len(self.param.src_host) > 1 and len(self.param.dst_host) == 1:
            # 传入flows<=0则表示自动判断流数
            if self.param.flows <= 0 or self.param.flow_cpu_ratio > 0:
                ratio = self.param.flow_cpu_ratio if self.param.flow_cpu_ratio else 0
                self.param.flows = Public.select_flow_from_host(self.param.dst_host[0], ratio)
                logger.log('host=%s flows=%d kpps=%d' % (self.param.dst_host[0], self.param.flows, self.param.set_kpps))
            # 是否设定了加压比例
            if self.param.per_host_kpps_ratio > 0:
                self.param.set_kpps = Public.select_kpps_from_flavor(self.param.dst_host[0],
                                                                     self.param.per_host_kpps_ratio)
            elif self.param.per_host_bw_ratio > 0:
                self.param.set_mb = Public.select_bw_from_flavor(self.param.dst_host[0],
                                                                 self.param.per_host_bw_ratio)
            # 然后设置-b参数
            if self.param.set_kpps > 0:
                self.param.flow_mb = self.select_iperf3_bw_from_flow_and_pps()
            elif self.param.set_mb > 0:
                self.param.flow_mb = self.param.set_mb * 1.0 / self.param.flows
            else:
                # PPS和BW都<=0，说明pps=0或者bw=0，不需要加压
                logger.log('no need to press')
                return RETURN_OK, client_cmd, server_cmd
            if DEBUG_MODE:
                logger.log('make_iperf3_cmd: self.param=%s' % repr(self.param.__dict__))
            # 转换VPC_IP
            host_eip = self.param.dst_host[0]
            host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
            per_flow = self.param.flows / len(self.param.src_host)
            left_flow = self.param.flows % len(self.param.src_host)
            for i in range(0, len(self.param.src_host)):
                for j in range(0, per_flow):
                    if test_type == 'TCP':
                        c_cmd = "iperf3 -c %s -p %d -b %.3fM -t %s 2>/dev/null 1>/dev/null &" \
                                % (host_vpc, port_index, float(self.param.flow_mb), self.param.test_time)
                    else:
                        c_cmd = "iperf3 -c %s -p %d -u -b %.3fM -t %s -l %d 2>/dev/null 1>/dev/null &" \
                                % (host_vpc, port_index, float(self.param.flow_mb),
                                   self.param.test_time, self.param.pkt_len)

                    s_cmd = 'iperf3 -s -p %d 2>/dev/null 1>/dev/null &' % port_index
                    port_index += 1
                    # 记录命令
                    for ele in client_cmd:
                        if ele['host'] == self.param.src_host[i]:
                            ele['cmd'].append(c_cmd)
                            break
                    for ele in server_cmd:
                        if ele['host'] == self.param.dst_host[0]:
                            ele['cmd'].append(s_cmd)
                            break

            for j in range(0, left_flow):
                # 转换VPC_IP
                host_eip = self.param.dst_host[-1]
                host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
                if test_type == 'TCP':
                    c_cmd = "iperf3 -c %s -p %d -b %.3fM -t %s 2>/dev/null 1>/dev/null &" \
                            % (host_vpc, port_index, float(self.param.flow_mb), self.param.test_time)
                else:
                    c_cmd = "iperf3 -c %s -p %d -u -b %.3fM -t %s -l %d 2>/dev/null 1>/dev/null &" \
                            % (host_vpc, port_index, float(self.param.flow_mb),
                               self.param.test_time, self.param.pkt_len)

                s_cmd = 'iperf3 -s -p %d 2>/dev/null 1>/dev/null &' % port_index
                port_index += 1
                # 记录命令
                for ele in client_cmd:
                    if ele['host'] == self.param.src_host[-1]:
                        ele['cmd'].append(c_cmd)
                        break
                for ele in server_cmd:
                    if ele['host'] == self.param.dst_host[0]:
                        ele['cmd'].append(s_cmd)
                        break
            pass
        # N:M复杂场景，不支持
        else:
            logger.log('not support scenarios')
            return RETURN_FAIL, client_cmd, server_cmd
        return RETURN_OK, client_cmd, server_cmd

    def call_iperf3(self):
        r, c, s = self.make_iperf3_cmd()
        if DEBUG_MODE:
            logger.log('call_iperf3: src=%s  dst=%s' % (str(self.param.src_host), str(self.param.dst_host)))
        if r == RETURN_FAIL:
            return RETURN_FAIL
        if DEBUG_MODE:
            logger.log('call_iperf3: start server')
        # 启动server
        for i in s:
            if i['cmd']:
                logger.log('host=%s cmd=%s' % (i['host'], str(i['cmd'])))
                Public.exec_shell_command(i['host'], i['cmd'])
                time.sleep(SleepTime.milli_sec)
        time.sleep(SleepTime.one_sec)
        # 启动client
        if DEBUG_MODE:
            logger.log('call_iperf3: start client')
        for i in c:
            if i['cmd']:
                logger.log('host=%s cmd=%s' % (i['host'], str(i['cmd'])))
                Public.exec_shell_command(i['host'], i['cmd'])
                time.sleep(SleepTime.milli_sec)

    def get_data(self):
        pass

    def stop(self):
        Public.off(self.param.src_host + self.param.dst_host, ['iperf3'])

    def run(self):
        """
        # 功能，运行iperf3命令
        :param prefix: 提示前缀
        """
        # 停止自己以前的测试
        self.stop()
        # 启动测试
        self.call_iperf3()


class QperfTester(object):
    def __init__(self, param=None):
        if param:
            self.param = copy.deepcopy(param)
        else:
            self.param = TestParam()
        # 初始化数据存储
        self.log_format = '/tmp/%s_qperf.log'
        self.data = {}
        self.log_list = {}

    def init_data(self):
        self.data = {}
        self.log_list = {}

    def make_qperf_cmd(self, src_host, dst_host):
        """
        # 函数功能：qperf命令生成器
        :return:
        """
        client_cmd = []
        server_cmd = []
        test_method_template = ['udp_lat', 'tcp_lat']
        test_type = self.param.test_type.lower()
        if DEBUG_MODE:
            logger.log('%s: begin' % sys._getframe().f_code.co_name)
        if DEBUG_MODE:
            logger.log('%s: src=%s  dst=%s  type=%s  time=%d  pkt_len=%d'
                      % (sys._getframe().f_code.co_name, str(src_host), str(dst_host),
                         self.param.test_type, self.param.test_time, self.param.pkt_len))
        if test_type not in test_method_template:  # 测试方法不对
            return RETURN_FAIL, client_cmd, server_cmd
        if len(src_host) == 0 or len(dst_host) == 0:  # 有一方为0
            return RETURN_FAIL, client_cmd, server_cmd
        if len(src_host) != len(dst_host):  # src与dst个数不等，不支持
            return RETURN_FAIL, client_cmd, server_cmd
        # 参数检查通过,初始命令字典
        for i in src_host:
            element = dict(host=i, cmd=[])
            client_cmd.append(element)
        for i in dst_host:
            element = dict(host=i, cmd=[])
            server_cmd.append(element)
        for client_eip,server_eip in zip(src_host, dst_host):
            # 转换VPC_IP
            server_vpc = EIP_VPC_IP[server_eip]['vpc'] if EIP_VPC_IP[server_eip]['vpc'] else server_eip
            qperf_log = self.log_format % client_eip
            if client_eip not in self.log_list.keys(): self.log_list[client_eip] = []
            self.log_list[client_eip].append(qperf_log)
            c_cmd = 'qperf %s -t %d -m %d -vu %s > %s &' \
                    % (server_vpc, self.param.test_time, self.param.pkt_len, self.param.test_type, qperf_log)
            s_cmd = 'qperf &'
            # 记录命令
            for ele in client_cmd:
                if ele['host'] == client_eip:
                    ele['cmd'].append(c_cmd)
                    break
            for ele in server_cmd:
                if ele['host'] == server_eip:
                    ele['cmd'].append(s_cmd)
                    break
        if DEBUG_MODE:
            logger.log('%s: done' % sys._getframe().f_code.co_name)
        return RETURN_OK, client_cmd, server_cmd

    def call_qperf(self, src_host, dst_host):
        r, c, s = self.make_qperf_cmd(src_host, dst_host)
        if DEBUG_MODE:
            logger.log('src=%s  dst=%s' % (str(src_host), str(dst_host)))
        if r == RETURN_FAIL:
            return RETURN_FAIL
        # 停止qperf
        Public.off(src_host + dst_host, ['qperf'])
        # 启动server
        for host in s:
            if DEBUG_MODE:
                logger.log('call_qperf: host=%s  cmd=%s' % (host['host'], str(host['cmd'])))
            Public.exec_shell_command(host['host'], host['cmd'], backgroud=True)
            time.sleep(SleepTime.milli_sec)
        time.sleep(SleepTime.ten_sec)
        # 启动client
        if DEBUG_MODE:
            logger.log('call_qperf: begin start client')
        for host in c:
            if DEBUG_MODE:
                logger.log('call_qperf: host=%s  cmd=%s' % (host['host'], str(host['cmd'])))
            Public.exec_shell_command(host['host'], host['cmd'])
            time.sleep(SleepTime.milli_sec)

    def fetch_data(self):
        # 采集数据
        time.sleep(SleepTime.ten_sec + self.param.test_time)
        logger.log('qperf: fetch_data begin')
        for host,logs in self.log_list.items():
            for each_log in logs:
                cmd = "script -q -c 'scp %s:%s %s' >/dev/null" % (host, each_log, each_log)
                os.system(cmd)
                if not os.path.isfile(each_log):
                    return RETURN_FAIL
                # 分析数据
                with open(each_log, 'r') as f:
                    lat = 0
                    unit = 'us'
                    for line in f:
                        line = line.strip()
                        if 'latency' in line:
                            lat_str = line.split()
                            lat = round(float(lat_str[2]), 1)
                            unit = lat_str[3]
                    if 'ms' in unit.lower():
                        lat = lat * 1000.
                    elif 'sec' in unit.lower():
                        lat = lat * 1000. * 1000.
                    # 存储数据
                    if host not in self.data.keys(): self.data[host] = {'type': self.param.test_type, 'lat': []}
                    self.data[host]['lat'].append(lat)
                # 清除临时文件
                if os.path.isfile(each_log):
                    os.remove(each_log)

    def run(self, prefix=''):
        """
        # 函数功能：测试qperf延迟
        """
        # 执行入口代码
        logger.log('%sqperf: src=%s dst=%s type=%s pkt_len=%d' %
                   (prefix, str(self.param.src_host), str(self.param.dst_host), self.param.test_type,
                    self.param.pkt_len))
        self.init_data()
        self.call_qperf(self.param.src_host, self.param.dst_host)


class PingTester(object):
    """ ping测试对象 """
    def __init__(self, src=None, dst=None, count=60, interval=1.0):
        self.src = src
        self.dst = dst
        self.count = count
        self.interval = interval
        self.data = {}
        self.init_data()
        self.log_format = '/tmp/%s_ping.log'
        self.log_list = {}

    def init_data(self):
        self.data = {}
        self.log_list = {}

    def _make_cmd(self):
        client_cmd = []
        # 初始化字典
        for i in self.src:
            element = dict(host=i, cmd=[])
            client_cmd.append(element)
        for client_eip,server_eip in zip(self.src, self.dst):
            # 转换VPC_IP
            host_eip = server_eip
            host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
            ping_log = self.log_format % client_eip
            if client_eip not in self.log_list.keys(): self.log_list[client_eip] = []
            self.log_list[client_eip].append(ping_log)
            c_cmd = 'ping %s -c %d -i %i > %s &' % (host_vpc, self.count, self.interval, ping_log)
            for ele in client_cmd:
                if ele['host'] == client_eip:
                    ele['cmd'].append(c_cmd)
                    break
        return RETURN_OK, client_cmd, None

    def fetch_data(self):
        # 等待采集结束
        time.sleep(SleepTime.ten_sec + int(self.count*self.interval))
        logger.log('ping: fetch_data')
        for host, logs in self.log_list.items():
            for each_log in logs:
                cmd = "script -q -c 'scp %s:%s %s' > /dev/null" % (host, each_log, each_log)
                os.system(cmd)
                # 分析数据
                if not os.path.isfile(each_log):
                    return RETURN_FAIL
                with open(each_log, 'r') as f:
                    count = 0
                    for line in f:
                        line = line.strip()
                        # 匹配
                        ret = re.search('icmp_seq=\d+[\w\W]*time=[\w\W]*', line)
                        if ret:
                            count += 1
                            ret_str = re.split("[ =]", ret.group())
                            try:
                                unit = ret_str[6]
                                lat = round(float(ret_str[5]), 3)
                            except ValueError as e:
                                lat = 0.0
                            except Exception as e:
                                lat = 0.0
                            if host not in self.data.keys(): self.data[host] = []
                            self.data[host].append(lat)
                # 移除临时文件
                if os.path.isfile(each_log):
                    os.remove(each_log)
        if DEBUG_MODE:
            logger.log('ping: fetch_data finish')

    def run(self, prefix=''):
        logger.log('%sping: start %s to %s' % (prefix, self.src, self.dst))
        r,c,s = self._make_cmd()
        if r == RETURN_FAIL: return RETURN_FAIL
        # 执行测试
        for host in c:
            if DEBUG_MODE:
                logger.log('run_ping: host=%s  cmd=%s' % (host['host'], str(host['cmd'])))
            Public.exec_shell_command(host['host'], host['cmd'], backgroud=True)


class MemcachedTester(object):
    """ 新浪memcached业务测试 """
    srv_file = '/root/%s_memcached.log'
    tmp_file = '/tmp/%s_memcached.log'

    def __init__(self, src_host=None, dst_host=None, port=22122, t_time=60, thread=16, vusers=256, t_byte=100):
        if src_host:
            self.src_host = src_host if type(src_host) is list else [src_host]
        if dst_host:
            self.dst_host = dst_host if type(dst_host) is list else [dst_host]
        self.port = port
        self.t_time = t_time
        self.thread = thread
        self.vusers = vusers
        self.t_byte = t_byte
        self.data = {}
        self.init_data()

    def init_data(self):
        self.data = {}

    def _make_cmd(self):
        """ 产生memcached测试命令 """
        client_cmd = []
        server_cmd = []
        # 初始命令字典
        for i in self.src_host:
            element = dict(host=i, cmd=[])
            client_cmd.append(element)
        for i in self.dst_host:
            element = dict(host=i, cmd=[])
            server_cmd.append(element)
        if DEBUG_MODE:
            logger.log('make_memcached_cmd begin')
        if len(self.src_host) != len(self.dst_host):
            logger.log('memcached error: len(src_host) != len(dst_host)')
            return RETURN_FAIL
        for client,server in zip(self.src_host, self.dst_host):
            # 转换VPC_IP
            host_eip = server
            host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
            c_cmd = 'memaslap -s %s:%s -t %ds -T %d -c %d -X %dB > %s &' %\
                    (host_vpc, self.port, self.t_time, self.thread, self.vusers, self.t_byte, self.srv_file % client)
            s_cmd = 'memcached -p %d -d -u root' % self.port
            # 记录命令
            for ele in client_cmd:
                if ele['host'] == client:
                    ele['cmd'].append(c_cmd)
                    break
            for ele in server_cmd:
                if ele['host'] == server:
                    ele['cmd'].append(s_cmd)
                    break
        # 返回命令结果
        return RETURN_OK, client_cmd, server_cmd

    def fetch_data(self):
        time.sleep(SleepTime.ten_sec + self.t_time + 5)
        logger.log('%s fetch_data begin' % self.__class__.__name__)
        # 拉取结果
        for host in self.src_host:
            cmd = "script -q -c 'scp %s:%s %s' > /dev/null" % (host, self.srv_file % host, self.tmp_file % host)
            os.system(cmd)
        # 分析数据
        for host in self.src_host:
            t_file = self.tmp_file % host
            if not os.path.isfile(t_file):
                return RETURN_FAIL
            tps = 0
            with open(t_file, 'r') as f:
                for line in f:
                    line = line.strip()
                    # 写入日志
                    # logger.log(line)
                    if 'TPS' in line:
                        ret_str = re.findall('TPS\W+\d+', line)[0].split(':')
                        try:
                            tps = int(ret_str[1].strip())
                        except Exception as e:
                            logger.log('tps value get error')
                            tps = 0
            # 加入数据仓库
            self.data[host] = tps
            if DEBUG_MODE:
                logger.log('host=%s  tps=%d' % (host, tps))
            # 清理临时文件
            if os.path.isfile(t_file):
                os.remove(t_file)

    def stop(self):
        Public.off(self.src_host + self.dst_host, ['memcached', 'memaslap'])

    def run(self, prefix=''):
        r,c,s = self._make_cmd()
        if r == RETURN_FAIL:
            return RETURN_FAIL
        logger.log('%sMemcachedTester run: %s --> %s' % (prefix, self.src_host, self.dst_host))
        for i in s:
            Public.exec_shell_command(i['host'], i['cmd'])
            time.sleep(SleepTime.milli_sec)
        time.sleep(SleepTime.ten_sec)
        for i in c:
            Public.exec_shell_command(i['host'], i['cmd'])
            time.sleep(SleepTime.milli_sec)
        if DEBUG_MODE:
            logger.log('MemcachedTester run: done')


def run_ssh_login_and_get_time(src_host_list=None, dst_host_list=None, test_time=100, sleep_gap=2,
                               test_order=ORDER_ALL, test_round=5):
    """
    # 函数功能：测试ssh时长
    :param src_host_list: 源IP列表
    :param dst_host_list: 目的IP列表
    :param test_time: 测试多少次
    :param sleep_gap: ssh等待
    :param test_order: 测试顺序ORDER_ALL=并行   ORDER_STEP=串行
    :param test_round: 循环次数
    """
    def call_ssh(src_host_list, dst_host_list):
        client_cmd = []
        ssh_avg_list = {}  # 数据格式{ '192.168.0.1':{'sum':0, 'avg':0, 'details':[1, 2, 3]} }
        if DEBUG_MODE:
            logger.log('call_ssh: begin make commands')
        if set(src_host_list) == set(dst_host_list):
            logger.log('warning: same src and dst,exit\nsrc=%s\ndst=%s' % (str(src_host_list), str(dst_host_list)))
            return
        for i in src_host_list:
            element = dict(host=i, cmd=[], file='')
            client_cmd.append(element)
            ssh_avg_list[i] = dict(sum=0, avg=0, details=[])
        # 构造命令
        for client,server in zip(src_host_list, dst_host_list):
            # 转换VPC_IP
            host_eip = server
            host_vpc = EIP_VPC_IP[host_eip]['vpc'] if EIP_VPC_IP[host_eip]['vpc'] else host_eip
            for ele in client_cmd:
                if ele['host'] == client:
                    ele['file'] = '/tmp/date_%s' % ele['host']
                    ele['cmd'].append('date +%%s.%%N > %s' % ele['file'])
                    ele['cmd'].append('ssh -o ServerAliveInterval=60 %s echo 2>/dev/null 1>/dev/null' % host_vpc)
                    ele['cmd'].append('date +%%s.%%N >> %s' % ele['file'])
                    break
        # 启动client
        for round_num in range(0, test_round):
            if DEBUG_MODE:
                logger.log('call_ssh: start client ssh')
            for tt in range(0, test_time):
                for i in client_cmd:
                    Public.exec_shell_command(i['host'], i['cmd'])
                # 搜集结果
                for i in client_cmd:
                    host = i['host']
                    os.system("script -q -c 'scp %s:%s %s' >/dev/null" % (host, i['file'], i['file']))
                    # os.system('cat %s' % i['file'])
                    with open(i['file'], 'r') as f:
                        time1 = float(f.readline().strip())
                        time2 = float(f.readline().strip())
                        diff_time = round(time2 - time1, 3)
                        ssh_avg_list[host]['sum'] += diff_time
                        ssh_avg_list[host]['details'].append(diff_time)
                        ssh_avg_list[host]['avg'] = ssh_avg_list[host]['sum']/len(ssh_avg_list[host]['details'])
                        logger.log('time=%.3f' % ssh_avg_list[host]['details'][-1])
                time.sleep(sleep_gap)
            # 打印平均
            for i in client_cmd:
                host = i['host']
                logger.log('avg_time=%.3f' % ssh_avg_list[host]['avg'])

            # 每个回合休息一段时间
            if round_num < test_round:
                time.sleep(SleepTime.one_min)

    # 只支持N:N
    if len(src_host_list) != len(dst_host_list):
        return RETURN_FAIL

    # 并行测试
    if test_order == ORDER_ALL:
        call_ssh(src_host_list, dst_host_list)
    # 串行测试
    else:
        for c,s in zip(src_host_list, dst_host_list):
            call_ssh([c], [s])


def run_netperf_iperf3_task(tester, repeat=1):
    """
    函数功能：netperf/iperf3业务带宽PPS测试
    :param tester: NetperfTester实例
    :param repeat: 重复几次
    """
    send_dict = {'eth': NIC_NAME, 'rxpps': [], 'txpps': [], 'rxbw': [], 'txbw': []}
    recv_dict = {'eth': NIC_NAME, 'rxpps': [], 'txpps': [], 'rxbw': [], 'txbw': []}
    # 测试次数
    for i in range(0, repeat):
        msg = ''
        app_name = ''
        app_list = ['netperf', 'iperf3']
        tmp_flow = tester.param.flows if tester.param.flows else 'auto'
        if tester.param.set_kpps > 0:
            tmp_set_mb = -1
        for app in app_list:
            if app in tester.__class__.__name__.lower():
                app_name = app
                break
        msg = 'Round %d/%d: %s src=%s, dst=%s, type=%s, flow=%s, set_mb=%sM, set_kpps=%d, pktlen=%d' % \
              ((i+1), repeat, app_name, tester.param.src_host, tester.param.dst_host, tester.param.test_type, str(tmp_flow),
               str(tester.param.set_mb), tester.param.set_kpps, tester.param.pkt_len)
        logger.log(msg)
        tester.run()
        tester.sar.run(sleep_time=SleepTime.half_min)
        tester.sar.fetch_data()
        # 拷贝数据
        tester.sar.copy_data(send_dict, recv_dict)
        if i < repeat-1:
            time.sleep(SleepTime.ten_sec)
    # 打印汇总结果
    # if len(tester.param.src_host) > 1:
    #     logger.log('\n=========================== send summary result ===========================')
    #     for index in range(0, len(send_dict['rxpps'])):
    #         eth = send_dict['eth']
    #         rxpps = send_dict['rxpps'][index]
    #         txpps = send_dict['txpps'][index]
    #         rxbw = send_dict['rxbw'][index]
    #         txbw = send_dict['txbw'][index]
    #         msg = tester.sar.sar_format.format('%ds' % (index+1), eth, rxpps, txpps, rxbw, txbw)
    #         logger.log(msg)
    #     logger.log('END')
    # if len(tester.param.dst_host) > 1:
    #     logger.log('\n=========================== recv summary result ===========================')
    #     for index in range(0, len(recv_dict['rxpps'])):
    #         eth = recv_dict['eth']
    #         rxpps = recv_dict['rxpps'][index]
    #         txpps = recv_dict['txpps'][index]
    #         rxbw = recv_dict['rxbw'][index]
    #         txbw = recv_dict['txbw'][index]
    #         msg = tester.sar.sar_format.format('%ds' % (index+1), eth, rxpps, txpps, rxbw, txbw)
    #         logger.log(msg)
    #     logger.log('END')
    # 测试完后停止打流
    # tester.stop()


def run_netperf_tcp_crr_task(tester, repeat=1):
    """
    函数功能：执行netperf TCP_CRR UDP_RR TCP_RR测试
    :param tester: NetperfTester实例
    :param repeat: 重复测几次
    :return:
    """
    send_dict = {}
    # 初始化字典
    for host in tester.param.src_host + tester.param.dst_host:
        send_dict[host] = {'type': tester.param.test_type, 'value': []}
    for i in range(0, repeat):
        msg = ''
        tmp_flow = tester.param.flows if tester.param.flows else 'auto'
        msg = 'Round %d/%d: %s src=%s dst=%s type=%s time=%d flow=%s pktlen=%d' % \
              ((i+1), repeat, tester.__class__.__name__, tester.param.src_host, tester.param.dst_host,
               tester.param.test_type, tester.param.test_time, str(tmp_flow), tester.param.pkt_len)
        logger.log(msg)
        tester.run()
        time.sleep(SleepTime.one_min + tester.param.test_time + 10)
        tester.fetch_data()
        # 计算每个ECS TCP_CRR以及总TCP_CRR
        sum_tcp_crr = 0
        for host,logs in tester.log_list.items():
            host_tcp_crr = 0
            for each_log in logs:
                with open(each_log, 'r') as f:
                    for line in f:
                        if re.findall('^\d+\s+\d+\s+\d+\s+\d+\s+', line) and str(tester.param.test_time) in line:
                            try:
                                host_tcp_crr += float(line.split()[-1])
                            except Exception as e:
                                host_tcp_crr += 0
                # 移除临时文件
                if os.path.isfile(each_log):
                    os.remove(each_log)
            # 所有主机TCP_CRR求和
            sum_tcp_crr += host_tcp_crr
            # 打印计算
            logger.log('tcp_crr on host=%s TCP_CRR=%.1f' % (host, host_tcp_crr))


def run_ping_task(tester, repeat=1):
    """
    函数功能：ping业务测试
    :param tester: PingTester实例
    :param repeat: 重复几次
    """
    for i in range(0, repeat):
        tester.init_data()
        tester.run(prefix='Round %d/%d    ' % (i+1, repeat))
        tester.fetch_data()
        # 打印结果
        for host, lats in tester.data.items():
            logger.log('ping host=%s' % host)
            for each_lat in lats:
                logger.log(each_lat)
        # 多次测试等待
        if i < repeat-1:
            time.sleep(SleepTime.ten_sec)


def run_qperf_task(tester, repeat=1):
    """
    函数功能：qperf业务测试
    :param tester: QperfTester实例
    :param repeat: 重复几次
    """
    for i in range(0, repeat):
        tester.init_data()
        tester.run(prefix='Round %d/%d    ' % (i+1, repeat))
        tester.fetch_data()
        # 打印结果
        for host,record in tester.data.items():
            logger.log('qperf on host=%s type=%s lat=%.1f us' % (host, record['type'], float(record['lat'][0])))
        # 多次测试等待
        if i < repeat-1:
            time.sleep(SleepTime.ten_sec)


def run_memcached_task(tester, repeat=1):
    for i in range(0, repeat):
        # 开始前停止干扰
        tester.stop()
        tester.run(prefix='Round %d/%d    ' % (i + 1, repeat))
        tester.fetch_data()
        # 打印结果
        for host,tps in tester.data.items():
            msg = 'memcached: host=%s TPS=%d' % (host, tps)
            logger.log(msg)
        # 多次测试等待
        if i < repeat-1:
            time.sleep(SleepTime.ten_sec)


def shutoff_ecs(hosts=None):
    """
    函数功能：关机脚本
    :param hosts:
    :return:
    """
    if hosts:
        if type(hosts) is str:
            hosts = [hosts]
    else:
        return RETURN_FAIL
    # 关机程序
    for host in hosts:
        Public.exec_shell_command(host, ['poweroff'])
        time.sleep(SleepTime.one_sec)


def main():
    # 主函数
    global DEBUG_MODE
    global logger
    task_app = ['netperf', 'qperf', 'ping', 'memcached', 'tcp_crr']
    # task_app = ['netperf', 'qperf', 'ping', 'iperf3', 'tcp_crr']
    test_order = ORDER_ALL
    repeat = 5
    ping_repeat = 1

    # 获取参数
    if TASK_APP != 'NULL':
        task_app = [TASK_APP]
    task_app = ['qperf']
    # 获取IP地址
    src_host_list = re.split("[ ;,]", SRC_HOST)
    dst_host_list = re.split("[ ;,]", DST_HOST)
    # 去列表空格元素
    for i in range(len(src_host_list)-1, 0, -1):
        if not src_host_list[i]:
            src_host_list.pop(i)
    for i in range(len(dst_host_list)-1, 0, -1):
        if not dst_host_list[i]:
            dst_host_list.pop(i)
    # 建立信任关系
    Public.set_ssh_trust(src_host_list+dst_host_list, PASSWORD_HASH)
    # 绑定EIP对应的私有IP
    Public.bind_eip_to_vpc(src_host_list + dst_host_list)
    # 停止打流
    Public.off(src_host_list + dst_host_list)
    # 打流方式
    if DEBUG_MODE:
        logger.log('main: test_type = %s' % task_app)
    if 'netperf' in task_app:
        logger.log('\n\n>>> task_app: netperf')
        # 先来一波TCP_STREAM带宽测试
        netperf_param1 = TestParam(None, None, 'TCP_STREAM', 7001, 0, 0, -1, 0, 7200, 1440)
        netperf_param1.set_param(src_host=src_host_list, dst_host=dst_host_list)
        netperf_tester1 = NetperfTester(netperf_param1)
        netperf_tester1.sar.set_param(src_host=src_host_list, dst_host=None, c_time=300, details=True, eth=NIC_NAME)
        run_netperf_iperf3_task(netperf_tester1, repeat)
    if 'tcp_crr' in task_app:
        logger.log('\n\n>>> task_app: tcp_crr')
        netperf_param2 = TestParam(None, None, 'TCP_CRR', 7001, 4, 0, -1, -1, 20, 64)
        netperf_param2.set_param(src_host=src_host_list, dst_host=dst_host_list)
        netperf_tester2 = NetperfTester(netperf_param2)
        run_netperf_tcp_crr_task(netperf_tester2, repeat)
    if 'iperf3' in task_app:
        logger.log('\n\n>>> task_app: iperf3')
        # 先测一波梯度pps，单位K
        for pps in range(0, 10, 4):
            iperf3_param1 = TestParam(None, None, 'udp', 8001, 0, 0, -1, set_kpps=pps, test_time=3600, pkt_len=64)
            iperf3_param1.set_param(src_host=src_host_list, dst_host=dst_host_list, test_time=3600)
            iperf3_test1 = Iperf3Tester(iperf3_param1)
            iperf3_test1.sar.set_param(src_host=src_host_list, c_time=300, details=True, eth=NIC_NAME)
            run_netperf_iperf3_task(iperf3_test1, repeat)
        # 再测一波带宽，单位Mb
        for bw in range(0, 300, 100):
            iperf3_param2 = TestParam(None, None, 'tcp', 8001, 0, 0, bw, set_kpps=-1, test_time=3600, pkt_len=0)
            iperf3_param2.set_param(src_host=src_host_list, dst_host=dst_host_list, test_time=3600)
            iperf3_test2 = Iperf3Tester(iperf3_param2)
            iperf3_test2.sar.set_param(src_host=src_host_list, c_time=300, details=True, eth=NIC_NAME)
            run_netperf_iperf3_task(iperf3_test2, repeat)
    if 'iperf3_bg' in task_app:
        # 背景加压
        logger.log('\n\n>>> task_app: iperf3_bg')
        ratio = RATIO/100.
        iperf3_bg_param = TestParam(src_host_list, dst_host_list, 'udp', 8001, 0, 1, -1, -1, 86400, 64, ratio, -1)
        iperf3_bg_tester = Iperf3Tester(iperf3_bg_param)
        msg = 'iperf3 bg press: src=%s dst=%s type=%s kpps_ratio=%.2f' %\
              (iperf3_bg_tester.param.src_host, iperf3_bg_tester.param.dst_host, iperf3_bg_tester.param.test_type,
               iperf3_bg_tester.param.per_host_kpps_ratio)
        logger.log(msg)
        iperf3_bg_tester.run()
    if 'qperf' in task_app:
        logger.log('\n\n>>> task_app: qperf')
        # 背景加压
        for pps in range(0, 1001, 200):
            iperf3_param1 = TestParam(None, None, 'udp', 8001, 0, 0, -1, set_kpps=pps, test_time=3600, pkt_len=64)
            iperf3_param1.set_param(src_host=src_host_list, dst_host=dst_host_list, test_time=3600)
            iperf3_test1 = Iperf3Tester(iperf3_param1)
            iperf3_test1.sar.set_param(src_host=src_host_list, c_time=10, details=True, eth=NIC_NAME)
            run_netperf_iperf3_task(iperf3_test1, 1)
            
            # ob to send
            time.sleep(10)
            logger.log('OB to send %s --> %s qperf' % (OB_HOST, src_host_list[0]))
            qperf_test1 = QperfTester()
            qperf_test1.param.set_param(OB_HOST, src_host_list[0], 'udp_lat', test_time=60, pkt_len=64)
            run_qperf_task(qperf_test1, repeat)
            # ob to recv
            time.sleep(10)
            logger.log('OB to recv %s --> %s qperf' % (OB_HOST, dst_host_list[0]))
            qperf_test2 = QperfTester()
            qperf_test2.param.set_param(OB_HOST, dst_host_list[0], 'udp_lat', test_time=60, pkt_len=64)
            run_qperf_task(qperf_test2, repeat)
            # send to recv
            time.sleep(10)
            logger.log('send to recv %s --> %s qperf' % (src_host_list[0], dst_host_list[0]))
            qperf_test3 = QperfTester()
            qperf_test3.param.set_param(src_host_list[0], dst_host_list[0], 'udp_lat', test_time=60, pkt_len=64)
            run_qperf_task(qperf_test3, repeat)
            # off
            iperf3_test1.stop()
        # qperf_tcp
        # qperf_test2 = QperfTester()
        # qperf_test2.param.set_param(src_host_list, dst_host_list, 'tcp_lat', test_time=10, pkt_len=64)
        # run_qperf_task(qperf_test2, repeat)
    if 'ssh' in task_app:
        logger.log('\n\n>>> task_app: ssh')
        run_ssh_login_and_get_time([OB_HOST], src_host_list, test_time=60, sleep_gap=2, test_round=2)
    if 'ping' in task_app:
        logger.log('\n\n>>> task_app: ping')
        ping_tester1 = PingTester(src=src_host_list, dst=dst_host_list, count=3600, interval=1.0)
        run_ping_task(ping_tester1, ping_repeat)
    if 'memcached' in task_app:
        logger.log('\n\n>>> task_app: memcached')
        memcached_tester1 = MemcachedTester(src_host_list, dst_host_list, 22122, 60, 16, 256, 100)
        run_memcached_task(memcached_tester1, repeat)


if __name__ == '__main__':
    OB_HOST = 'auto'
    NIC_NAME = 'eth0'
    LOG_FILE = ''
    EIP_VPC_IP = {}
    # 创建日志对象
    logger = Logger()
    # 解析参数
    parse_args()
    # 检查依赖
    if check_depends() == RETURN_FAIL:
        sys.exit(-1)
    if DEBUG_MODE:
        print 'BASE_DIR = %s' % os.path.abspath(BASE_DIR)
    # 正式运行
    main()
    logger.log('>>>finish')
